...
Route by tier: fable for hard adversarial judgment (the `/review` panel and `oracle-reviewer`), opus for analysis, design, and recomputation, sonnet for navigation and gathering including the verification auditors, haiku only for genuinely mechanical extraction. Two ceilings bound effort, and they are Erfan's, not a judgment about the work: **a fable agent never goes above `high`**, and opus may go to `xhigh` where the depth is earned, which is why `stat-aggregation-auditor` and `anti-confound-designer` sit there: recomputing a load-bearing contrast and assembling a control battery are the two places extra depth actually cashes out. Within those bounds effort follows the job rather than the tier, and the spread is deliberate: `high` for the five adversarial agents, for `paper-digest`, which mostly reads and summarizes, and for the citation judgment that decides whether a paper supports a claim; `medium` for resolve-and-compare verification and for gathering; `low` for `dataset-verifier`, which is a mechanical readiness check. Do not flatten these back to one value, and do not raise a fable agent past `high` on the argument that detection depth would benefit.
