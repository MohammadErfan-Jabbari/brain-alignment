...
**Last updated:** 2026-07-16 (primary-source novelty audit)
...
The literature measures LLM-brain alignment and now also uses brain data to tune speech and text models. Feature distillation, privileged-information distillation, and on-policy hidden-state supervision are also active, so neither “brain data can guide a model” nor “training-time privileged information can help a smaller LM” remains an available broad novelty claim. [Vaidya et al., 2026](literature/canonical/vaidya-2026_slow-fmri-fast-ecog-transfer.md) additionally report representation transfer from fMRI-tuned WavLM to different ECoG participants hearing different content, so cross-participant, cross-stimulus, and cross-modality transfer is no longer an available first. The narrow unresolved question is whether a biological or synthetic-neural privileged target contributes incremental, target-specific value to a fixed smaller student beyond a geometry-and-learnability-matched nonbrain objective, under correct seed and participant inference, and then survives an independent biological or practical endpoint. E016 directly tests part of that sequence and finds proxy optimization without real-brain transfer, shifting the contribution from a new positive algorithm to the intervention-identification and transfer controls needed to prevent a proxy success from becoming a biological overclaim.
...
| [Vaidya et al. 2026](literature/canonical/vaidya-2026_slow-fmri-fast-ecog-transfer.md) (arXiv) | `vaidya-2026_slow-fmri-fast-ecog-transfer` | WavLM Base+ models fine-tuned separately on three deeply sampled fMRI participants improve prediction of Podcast ECoG responses from nine different patients hearing different content. This closes the broad first for participant-, stimulus-, and modality-crossing representation transfer. The comparison is primarily against pretrained WavLM, with no neural permutation, learning-matched nonbrain target, training seeds, or participant-level primary inference; paper-specific code and checkpoints are not linked. It is the strongest prospective external adjudication target for the intervention-validity framework. |
| [Cheng et al. 2026](literature/canonical/cheng-2026_abstraction-induces-brain-alignment.md) (arXiv/OpenReview) | `cheng-2026_abstraction-induces-brain-alignment` | Nonlinear intrinsic dimension tracks layerwise fMRI/ECoG encoding and semantic abstraction across model families and training checkpoints; fMRI-tuning WavLM for two participants moves intrinsic dimension and semantic content together. High-dimensional random Fourier features remain weak encoders, so dimension is not sufficient. For target comparability, matching output width or rank alone is inadequate; spectrum, effective rank, nonlinear intrinsic dimension, learnability, and optimization pressure must also be audited. |
...
- [Prince et al. 2022](literature/canonical/prince-2022_glmsingle.md): single-trial fMRI estimation can improve through cross-validated voxelwise HRF selection, nuisance components, and fractional ridge; response estimation is therefore an upstream intervention, not a fixed preprocessing detail.
- [Li et al. 2019](literature/canonical/li-2019_learning-brains-regularize-machines.md): a learned neural system-identification model can denoise neural responses before they define a representational regularizer; the raw neural-similarity target did not reproduce the same gain.
- [Hari et al. 2026](literature/canonical/hari-2026_independent-component-encoding-models.md): subject-specific ICA can concentrate story-predictive fMRI structure into an auditable latent target. The reported component ranking uses held-out-test predictivity, so a training target must estimate and select components inside builder folds and still beat matched voxel/ROI, stimulus, acoustic, and shifted controls.
- [Diedrichsen et al. 2020](literature/canonical/diedrichsen-2020_whitened-unbiased-rdm-similarity.md): repeat-crossvalidated unbiased distances plus RDM-error whitening provide a candidate target when coordinate-wise voxel MSE is dominated by noise or arbitrary measurement bases.
- [Schütt et al. 2021](literature/canonical/schutt-2021_statistical-inference-representational-geometries.md): representational model fitting and inference must separate condition and participant generalization; flexible target models belong inside nested crossvalidation.
- [Moussa and Toneva 2025](literature/canonical/moussa-2025b_multi-participant-brain-tuning.md): participant-specific losses with a shared projection can outperform response averaging, making participant preservation a distinct design axis from denoising.
- [Koskinen and Seppä 2014](literature/canonical/koskinen-2014_task-optimized-meg.md): repeat agreement can supervise one participant-specific MEG spatial filter that transfers to held-out repeats and nonrepeated stories. Its 33-training/15-test repeat regime and multi-stage pipeline are much richer than E032, so the reported up-to-20-fold improvement is precedent, not a Kymata forecast.
- [Parra et al. 2019](literature/canonical/parra-2019_correlated-components-analysis.md): CorrCA formalizes the common-filter between-repeat/within-repeat covariance objective and its regularization. For E032, one shrinkage-regularized component, builder-only selection, cross-run evaluation, PCA-1, and shifted controls turn it into an auditable information-recovery assay.
- [Soto et al. 2026](literature/canonical/soto-2026_kymata-soto.md): Kymata supplies simultaneous repeated MEG and EEG for the same naturalistic conversation. The live archive is promising but not endpoint-ready: E2's event-duration inconsistency, acquisition/calibration conflict, channel-count mismatch, and license mismatch require a raw-header and trigger integrity gate before any neural score.
...
### F. Prospective external adjudication

The open gap is not another retrospective checklist. It is a prospective test of a published positive that first reproduces the source result and then applies predeclared intervention-specific controls. [Vaidya et al., 2026](literature/canonical/vaidya-2026_slow-fmri-fast-ecog-transfer.md) is the strongest target: obtain its code and checkpoints, reproduce the electrode-level fMRI-to-ECoG result, then test participant-level inference, nested lag selection, repeated training seeds, and geometry-and-learnability-matched auxiliary targets. If the artifacts remain unavailable, a reconstruction from the public datasets must be labeled a reimplementation rather than an exact reproduction.
