...
- **Fresh-session verifier mechanics (use for P2-B onward):** `CLAUDE_WRAP_SNAPSHOT_SKIP=1 timeout 540 claude -p "<task>" --model <m> --permission-mode bypassPermissions --add-dir <scratch>`; **raise the Bash tool's `timeout` to ≥540000ms** or the default 120s kills the run (this bit once). **`CLAUDE_WRAP_SNAPSHOT_SKIP=1` is required** — without it each verifier child fires SessionStart and clobbers `.claude/state/wrap/start.json`, forcing `/wrap` onto its fallback (fixed S36 — the hook now no-ops when the var is set). A fresh `claude -p` DOES load skills/commands/scripts added this session; an in-process Agent subagent does NOT (L061). Run the opus oracle review FIRST, fix, THEN the fresh-session verify.
