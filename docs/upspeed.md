...
9. **Manuscript → v0.9, COMPLETE-AS-ARTIFACT:** added a References section (21 entries; all verified vs primary sources by lit-scout — author lists corrected, Hoak→real vision-domain paper); a completeness-critic pass closed orphan citations + fixed dangling §-pointers. Every in-text citation resolves; gate = READY (thesis/workshop).
10. **Mock peer-review + hardening (S8-cont-2):** ran the COMPLETE v0.9 through 3 fable reviewers (oracle+premortem+counter-argument) → PASS thesis / borderline-PASS workshop. Fixed 5 abstract overclaims (the "well-powered zero in *every individual brain*" category error chief — rescoped to across-subject MEAN). Built a **real-residual bootstrap power positive-control** (`sensitivity_e008.py`): propagates the real within-subject noise the old sim ignored → group test power 0.92@uniform-δ=+0.002, FPR 0.01; first-principles VALID → defends the load-bearing null. Ran **E015** (`run_ppl_alignment_law.py`): cross-family alignment∝−ppl law **r≈−0.92** (gpt2/pythia/Qwen, n=8; robust to fair-probe + best-layer) generalizing E003's within-gpt2 r=−0.88 → the matched-ppl control now bites cross-family; folded into §1+evidence-map (L030; counter-argument caught my first broken-probe run + predicted the fix). **Premortem's strategic reframe** (move headline → matched-ppl-as-missing-control, demo on an external result) recorded as the open Erfan/fresh-build decision (L029).
