...
20. **A twin diff cannot see a fixed term drifting inside one tree, because both trees copy the drift.** Rule 14 checks that the derivative does not rename a term relative to the canonical tree. It says nothing about whether the canonical tree is consistent with its own registered short form, and a derivative built by copying inherits the inconsistency as agreement. Check each registered term against its own definition and declared short form within a single tree before comparing trees. Note that the terminology contract permits target dimension, dataset, model, layer, and response construction as qualifiers, so a qualified short form is compliant and a third independent name is not. *Evidence: proposed by `manuscript-twin-auditor` on the 2026-09-01 re-audit, on `03_experimental_framework.tex:36` (`50-component target-projection assay`) against the same file's line 208 definition; that instance is compliant under the qualifier clause, and the blind spot it exposes is not.*
