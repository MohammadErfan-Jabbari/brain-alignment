...
6. Apply accepted content edits to the canonical rewrite, propagate formatting-neutral changes deliberately to the submission derivative (respecting known variant spots), run `uv run python scripts/manuscript_check.py docs/manuscript/rewrite`, and rebuild both PDFs once per accepted subsection batch before committing atomically.
