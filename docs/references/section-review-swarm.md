...
Model routing is the session's declared constraint. v3.1 (Section 4, Erfan 2026-08-29): workers = deepseek-v4-flash + kimi-k3 (one analyst and one counter per scope per model), judges = kimi-k3:high + deepseek-v4-flash:high. v3.3 (Section 5, 2026-08-31): workers = deepseek-v4-flash:medium + ollama-cloud glm-5.3:medium; judges = GPT-5.6 Luna:high + GPT-5.6 Sol:high; Luna performs the blind pass and Sol the resolution judge. Earlier rounds used glm-5.3-flash workers and a glm-5.3 judge, but ollama-cloud glm children proved unreliable in the judge role: the judge burned its entire 32k output cap on `:high` thinking and produced no output twice, and one counter derailed into off-task content. The current route therefore uses GLM only as a bounded worker. Lesson: a judge model needs enough output-token headroom for the full decision list after thinking, and the thinking level must be set explicitly, never inherited.
