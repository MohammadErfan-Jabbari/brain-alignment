...
- Obsidian hides dotfolders; after the 2026-07-03 cleanup, the repo no longer exposes `.claude` through root symlinks. Use `.claude/agents`, `.claude/skills`, and `.claude/hooks` directly from the filesystem when needed. `.claude/worktrees` is a harness-owned local-only surface, not shared vault content.
