...
1. Continue the reader-first, section-by-section review from Section 2, preserving the conceptual-to-operational handoffs in the maintained question map; rebuild and commit each approved section-level unit.
2. Immediately after the manuscript review, design and make a go/no-go decision on a prospective dimensionality experiment: within one language-model lineage, test whether measured intrinsic or effective dimensionality of middle-layer representations explains controlled brain predictivity beyond tokenizer-fair language-model quality and parameter count, and whether that relationship saturates. Before any compute, complete the literature audit, define the estimand, estimator, common-support model set, dimensionality measure, controls, inference unit, uncertainty, and kill criteria, then run the required anti-confound and oracle precheck. Do not interpret 3B parameters as a human-brain capacity or treat parameter count, hidden width, intrinsic dimension, and optimization-space dimension as interchangeable.
