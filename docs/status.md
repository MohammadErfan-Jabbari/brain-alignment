...
**Manuscript:** `Section 5 prose swarm running (DeepSeek + Ollama Kimi K3 route)`. The 70-page thesis in `docs/manuscript/rewrite/` remains the sole scientific-interpretation authority; `docs/manuscript/submission/` is its university-template derivative. Introduction through Results have completed the reader-first review, and the six blocking Discussion-through-Conclusion [argument findings](manuscript/submission/deferred-review-items.md#6-discussion-through-conclusion-argument-review-hold) are resolved in both trees without changing a scientific verdict or number. Three launches stopped at the fail-loud phase gate on provider lockouts (GLM output-cap deaths; OpenAI-Codex usage limit, those models since removed; kimi-coding weekly quota). Per Erfan's 2026-08-31 instruction, kimi is now served by ollama-cloud/kimi-k3, and the swarm runs the proven Section-4 role structure with DeepSeek and Kimi. Under [D067](decisions/decisions.md) the parent applied 15 corroborated rule-backed DeepSeek normalizations canonical-first across three analyst runs (fixed role terms, settled phrasings, register, banned words); the judgment-call findings await this full dual-model swarm. Its professor-approved title is *Evaluating Brain Alignment as a Training Signal for Language Models* ([D066](decisions/decisions.md)).
...
- No scientific-evidence, argument-review, or operational blocker remains for the Section 5 prose swarm: the openai-codex models are removed, kimi is served by ollama-cloud/kimi-k3, and DeepSeek plus Ollama Kimi are both available. Ollama-cloud GLM stays unrouted after repeated output-cap deaths. Creating a new immutable public cut remains a separate decision requiring Erfan's approval.
...
1. Complete the hardened v3.3 Section 5 prose swarm (running on the DeepSeek + Ollama Kimi K3 route), then run the Limitations and Conclusion swarms, then close the deferred abstract and figure/typography passes.
