...
- H002 and E031 now define the prospective biological-information recovery direction. Tuckute cross-participant agreement is source consensus, not measurement reliability, and LeBel repeat averaging is a separate fixed-content diagnostic. No E031 endpoint has been opened. C1 compares a nested consensus-weighted target with a byte-identical uniform residual target in five target-construction-held-out participants; its matched contextual-text surrogate is frozen from six E025 KD-only caches. C2 varies builder repeat count against a disjoint repeat mean and has a proposed builder-only instrument floor and `10%` conditional response rule. The recorded E030 variance calibration shows that five-person simultaneous inference cannot resolve an effect near `+0.002`, so C1 is a fixed-cohort engineering gate and cannot support a participant-population `PASS`. The exact manifest, nine-test synthetic preflight, implementation review, and independent oracle re-audit now pass outcome-blind. A 2026-07-29 primary-source scout froze the external sequence as Kymata Soto repeat-method selection, MEG-MASC frozen replication, Caption Scene Dataset fMRI transfer, then Brain Treebank temporal validation; cross-modal datasets remain later priors rather than denoising evidence. Kymata's metadata gate passes. MEG-MASC is restricted to its current ten-person paired public subset. CSD's released GLMsingle betas fail the independent-repeat requirement because all 93 fitted run groups mix both presentations; a decisive CSD test would require repeat-isolated response re-estimation. Participant neural files remain unopened. The biological stages still fail closed. Erfan must explicitly accept or change the C2 constants and then authorize `/work E031`.
...
1. Decide whether to accept E031's proposed LeBel allocation constants: builder instrument floor `R_builder < 0.01` and conditional response floor `delta_C2 = 0.10 R_builder`. If accepted, explicitly authorize `/work E031`; otherwise revise the constants before any endpoint. Keep C1 scoped to the fixed five-person cohort. Do not open Kymata, MEG-MASC, or CSD participant neural files before the local mechanism gate.
