...
**2026-07-07 positive-branch pressure update.** A third focused Firecrawl Research pass expanded from OPRD/PHF/DemoPSD/Rethinking OPSD and read in-body passages from AR-OPD, EDGE-OPD, When Context Returns, and OPRD. The new detail is not a new immediate baseline; it is a sharper reviewer objection. Privileged teachers can push students toward locally unsupported shortcuts, rare-token support failures, side-effect tokens, or context-induced degradation. If E016's real-brain gate becomes positive, the next burden is to show that the brain-derived target produces transferable student change, not merely proxy fit or hindsight-biased privileged imitation.
...
| [Zhang et al., 2026, "Anchored Residual Guidance for Privileged On-Policy Distillation"](https://arxiv.org/abs/2606.10385) | Full-view privileged teachers can assign probability mass to answer-conditioned or future-conditioned states outside the student's local support; AR-OPD separates a reachable anchor from a controlled residual. | A future E016 positive must argue that the brain-derived target is locally learnable and transfer-relevant, not a hindsight-biased shortcut target. | AR-OPD studies oracle-trace privileged OPD on reasoning, not brain/fMRI/cognitive targets or fixed off-policy KD. |
| [Lazaridis et al., 2026, "EDGE-OPD"](https://arxiv.org/abs/2605.23493) | Privileged context can change behavior more than intended; EDGE uses guided rollouts plus an evidence mask so only tokens positively supported by the privileged context enter the loss. | A dense target loss over all target dimensions may need reviewer-facing justification that the transferable signal is not mixed with side-effect or unsupported privileged information. | EDGE is rare-token/persona and math OPD, not neural target KD; it suggests diagnostics rather than an immediate E016 cache-control. |
| [Wang et al., 2026, "When Context Returns"](https://arxiv.org/abs/2606.11627) | Successful context internalization can still be brittle: reintroducing the original privileged context can degrade accuracy; the paper names context removability as a robustness property. | This supports keeping real-brain transfer and robustness gates load-bearing instead of treating synthetic target fit as enough. | The setting is context distillation, not brain-derived targets; the analogue is transfer/robustness, not a direct baseline. |
