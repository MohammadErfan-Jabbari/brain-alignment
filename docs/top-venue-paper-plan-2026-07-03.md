...
Current route as of 2026-07-03: the full TRIBE artifact passed the analyzer/readiness gate and routed to `tribe_positive_needs_textfeat`. That route is a synthetic-target handoff, not a final verdict and not brain-specific evidence. The matched-information `textfeat` control is now running.
...
| Text-feature target | Matched-information non-brain privileged target: sentence-local frozen `gpt2-medium` hidden states, projected to the same target dimension. | Full textfeat train/heldout caches are built and validated; launcher is running. It clears the teacher-hidden-state/dense-feature alternative, not long-context or on-policy privileged distillation. |
| Text-feature block-permuted | Dense non-brain target-statistics twin. | Full textfeat launcher is running; no run JSON or analyzer JSON yet. |
