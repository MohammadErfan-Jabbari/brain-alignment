...
Current route as of 2026-07-07: the full TRIBE artifact passed the analyzer/readiness gate, the matched-information `textfeat` control completed, the comparator routed to `tribe_stronger_than_textfeat_needs_review`, and a local raw-row `/interpret` audit reproduced the TRIBE-minus-textfeat margins. That route supports a narrow post-positive branch, not a final verdict and not brain-specific clearance. Extra artifact-saving seeds `3,4,5` are running for both TRIBE and textfeat.
...
| TRIBE-vs-textfeat comparator | Read-only post-positive analyzer comparison. | `phase3_full_tribe_vs_textfeat_comparison.json` is ready and routes to `tribe_stronger_than_textfeat_needs_review`; `phase3_full_tribe_vs_textfeat_comparison.interpret_audit.json` reproduced the raw-row margins. |
...
| Extra seeds | Inference-strengthening if the positive branch survives. | Active: extra artifact-saving seeds `3,4,5` launched for TRIBE on GPU 1 and textfeat on GPU 2. Monitor to analyzer/readiness JSONs, then merge/compare seeds `0-5`. |
