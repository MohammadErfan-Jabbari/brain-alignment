...
> **↳ Correction (R04 full-read, 2026-06-10) — both mechanism stories.** **Story 1 (abstraction / LID).** *Original claim:* Cheng 2026 says local intrinsic dimension predicts brain-predictivity (and brain-tuning causally raises it), and "independently, lower-intrinsic-dimension representations generalize better and align better (2601.22722)" — cited as if the two agree. *What we did:* full read of `cheng-2026` (2602.04081) and `yu-2026` (2601.22722). *Corrected:* **they point in opposite directions, and both use *local* ID** — Cheng (language/speech, GRIDE estimator) finds **higher** local LID → more brain-aligned; Yu (vision, Levina–Bickel MLE) finds **lower** local LID → more aligned and more generalizable. "Local vs global" does not reconcile them; the sign is modality/estimator/frame-dependent. Cheng's *causal* claim is also thin — one model, one layer, two subjects, with an RFF control showing high LID *alone* is insufficient without learned linguistic structure. **Consequence for F3:** an intrinsic-dimension surrogate has **no agreed direction to optimize yet** — validate the sign against real fMRI before building on it. **Story 2 (neural-data regularizer / robustness).** *Original claim:* "a neural-data (monkey **IT**) regularizer via Deep CCA improved CNN accuracy *and* adversarial robustness (**Federer et al. 2022**)." *What we did:* full read of arXiv 2209.02582 (`pirlot-2022`). *Corrected:* the paper is **Pirlot et al. 2022** (Federer 2020 is the predecessor), using monkey **V1**, not IT. It does improve accuracy (+4.9pp on CIFAR-100) and FGSM robustness — **but a shuffled-label control reproduces the accuracy gain within 0.4%**, so the accuracy / sample-efficiency benefit there **does not require real neural structure**; only the *robustness* gain does. That weakens it as a precedent for F2's sample-efficiency claim (a reason R04 re-weights F2 below F1).
...
> **↳ Correction (R04 full-read, 2026-06-10) — brake (c), the compression counter-evidence.** *Original claim:* "post-hoc compression already preserves alignment by default (arXiv 2602.07547)." *What we did:* full read / rewrite of 2602.07547 (`oota-2026`). *Corrected:* alignment is preserved **only for AWQ/SmoothQuant on ≥3B models**; **GPTQ degrades** it (Δ≈−0.020, p<0.001), and **1–1.5B models and 50% pruning degrade** it too — not "by default." Decisively, the paper tests only **post-hoc quantization + pruning and never KD/distillation**: it lives in *shrink × measure*, a different cell from F1's *shrink × optimize*, so it **does not make F1 a non-problem** — it sets the trade-off-curve backdrop. Bookkeeping: **arXiv 2602.07547 *is* the same paper as the landscape's `oota-2026` note** (one work, two names) — don't double-count it.
...
- **Causal necessity** — Merlin &amp; Toneva, *ICLR 2026* (arXiv 2603.23091): models trained to be *brain-misaligned at matched perplexity* do substantially worse across 200+ downstream tasks. Brain alignment is load-bearing, causally, not just correlated.
  ...
  > **↳ Correction (R04 full-read, 2026-06-10).** *Original claim:* misalignment at matched perplexity "does substantially worse across 200+ downstream tasks; alignment is load-bearing, causally." *What we did:* full read of arXiv 2603.23091, *"When Language Models Lose Their Mind"* (`merlin-2026`; text models BERT / GPT-2 / Llama-3.2-1B, gradient-reversal de-alignment). *Corrected:* broadly correct, with one caveat and one understatement. **Caveat:** the damage is **not uniform** (e.g. GPT-2 on Harry-Potter fMRI is only marginal, p=0.055, n.s.). **Understatement:** the **positive arm** shows brain-tuning *up* significantly **helps** downstream — so the paper supports **A3-in-principle (induced alignment buys utility)** more strongly than "misalignment hurts" alone. All at constant parameter count, never under compression — it motivates F1's payoff without occupying its cell.
