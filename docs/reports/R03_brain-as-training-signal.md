...
**New to the vocabulary** (voxel/ROI, encoding model, unique R², noise ceiling and "% of ceiling", the trained−untrained gap)? The plain-language definitions live in [`../07-concepts-primer.md`](../07-concepts-primer.md); §2 below is the formal information-theory treatment.
...
> **↳ Correction (R04 full-read, 2026-06-10) — both mechanism stories.** **Story 1 (abstraction / LID).** *Original claim:* [Cheng 2026](../literature/canonical/cheng-2026_abstraction-induces-brain-alignment.md) says local intrinsic dimension predicts brain-predictivity (and brain-tuning causally raises it), and "independently, lower-intrinsic-dimension representations generalize better and align better (2601.22722)" — cited as if the two agree. *What we did:* full read of [`cheng-2026`](../literature/canonical/cheng-2026_abstraction-induces-brain-alignment.md) (2602.04081) and [`yu-2026`](../literature/canonical/yu-2026_intrinsic-dimension-alignment-generalization.md) (2601.22722). *Corrected:* **they point in opposite directions, and both use *local* ID** — Cheng (language/speech, GRIDE estimator) finds **higher** local LID → more brain-aligned; Yu (vision, Levina–Bickel MLE) finds **lower** local LID → more aligned and more generalizable. "Local vs global" does not reconcile them; the sign is modality/estimator/frame-dependent. Cheng's *causal* claim is also thin — one model, one layer, two subjects, with an RFF control showing high LID *alone* is insufficient without learned linguistic structure. **Consequence for F3:** an intrinsic-dimension surrogate has **no agreed direction to optimize yet** — validate the sign against real fMRI before building on it. **Story 2 (neural-data regularizer / robustness).** *Original claim:* "a neural-data (monkey **IT**) regularizer via Deep CCA improved CNN accuracy *and* adversarial robustness (**Federer et al. 2022**)." *What we did:* full read of arXiv 2209.02582 ([`pirlot-2022`](../literature/canonical/pirlot-2022_dcca-neural-regularizer-cnns.md)). *Corrected:* the paper is **[Pirlot et al. 2022](../literature/canonical/pirlot-2022_dcca-neural-regularizer-cnns.md)** (Federer 2020 is the predecessor), using monkey **V1**, not IT. It does improve accuracy (+4.9pp on CIFAR-100) and FGSM robustness — **but a shuffled-label control reproduces the accuracy gain within 0.4%**, so the accuracy / sample-efficiency benefit there **does not require real neural structure**; only the *robustness* gain does. That weakens it as a precedent for F2's sample-efficiency claim (a reason R04 re-weights F2 below F1).

**Where the mechanism breaks (the brakes, mandatory).** (a) If the brain-predictive structure is mostly *nuisance* — sentence length, position, low-level lexical identity — then aligning to it teaches nothing useful; this is the [Feghhi 2024](../literature/canonical/feghhi-2024_case-against-over-reliance-brain-scores.md) / [Oota 2024](../literature/canonical/oota-2024_speech-lms-lack-brain-semantics.md) brake and is exactly why every number in this thesis must be a *unique* variance after nuisance subtraction under contiguous splits (L003). (b) If the abstraction geometry is already reached by scale alone — brain alignment *saturates* around 3B parameters (Oota 2026) — then the bias is redundant above that scale. (c) If post-hoc compression already preserves alignment by default (arXiv 2602.07547, the key 2026 counter-evidence), then "protect alignment during compression" may be solving a non-problem unless framed as a *trade-off curve*, not a *preserve-vs-destroy* binary. These three brakes jointly pin the value of the idea to a specific corner: **small / under-trained / compressed models, in low-data regimes** — precisely where a weak-but-well-structured prior matters most.

> **↳ Correction (R04 full-read, 2026-06-10) — brake (c), the compression counter-evidence.** *Original claim:* "post-hoc compression already preserves alignment by default (arXiv 2602.07547)." *What we did:* full read / rewrite of 2602.07547 ([`oota-2026`](../literature/canonical/oota-2026_brain-encoding-scale-compression.md)). *Corrected:* alignment is preserved **only for AWQ/SmoothQuant on ≥3B models**; **GPTQ degrades** it (Δ≈−0.020, p<0.001), and **1–1.5B models and 50% pruning degrade** it too — not "by default." Decisively, the paper tests only **post-hoc quantization + pruning and never KD/distillation**: it lives in *shrink × measure*, a different cell from F1's *shrink × optimize*, so it **does not make F1 a non-problem** — it sets the trade-off-curve backdrop. Bookkeeping: **arXiv 2602.07547 *is* the same paper as the landscape's `oota-2026` note** (one work, two names) — don't double-count it.
...
- **Brain-tuning, speech** — Moussa, Klakow, Toneva, *ICLR 2025* ([`moussa-2025_brain-tuning-speech-lms`](../literature/canonical/moussa-2025_brain-tuning-speech-lms.md)): an L2 voxelwise fMRI loss fine-tunes ~90M speech LMs, +30% late-language alignment, consistent downstream gains, using &lt;0.7% of training data. Follow-up *NeurIPS 2025* (arXiv 2510.21520): multi-participant LoRA brain-tuning, up to +50% alignment, 5× fMRI-data efficiency.

  > **↳ Correction (R04 full-read, 2026-06-10).** *Original claim:* "+30% late-language alignment, consistent downstream gains, <0.7% data." *What we did:* full-PDF read of [`moussa-2025`](../literature/canonical/moussa-2025_brain-tuning-speech-lms.md) and the NeurIPS'25 follow-up [`moussa-2025b`](../literature/canonical/moussa-2025b_multi-participant-brain-tuning.md). *Corrected:* the +30% late-language gain holds for **Wav2Vec2.0 and HuBERT but not Whisper** (no significant late-language gain there), and downstream improves on **5/6 tasks** (emotion recognition is the exception). The ~90M, L2-voxelwise and <0.7%-data figures are correct. The NeurIPS follow-up's +50% / ~5× numbers are correct, but it trains on **3 participants** — the "88 participants" in an earlier draft of the canonical note was a hallucination, now fixed.
- **Brain-tuning, text** — Bilgin, Wehbe et al., *ICLR 2026* (OpenReview 07S1CPoQYP): fMRI-augmented training of **GPT-2 (124M) and LLaMA-2 (7B)** on Friends fMRI beats text-only baselines, scales with model size, generalizes across subjects/stimuli, and helps VL-Commonsense. This is the inversion, *for text LMs*, already published.
  ...
  > **↳ Correction (R04 full-read, 2026-06-10).** *Original claim:* GPT-2 124M + LLaMA-2 7B "beats text-only, scales with model size, generalizes across subjects/stimuli, helps VL-Commonsense." *What we did:* full read of the 14 MB OpenReview PDF ([`bilgin-2026`](../literature/canonical/bilgin-2026_brain-informed-lm-training.md)). *Corrected:* the direction is right — the **text-LM inversion is genuinely scooped** — but four specifics need fixing: (1) the brain loss is **cosine similarity, not L2**; (2) the VL-Commonsense (CoDa) gain is **LLaMA-2-only** (0.560→0.584); **GPT-2 regresses** (0.381→0.375); (3) "scales with model size" rests on **two model sizes, not a curve**; (4) train/val splits are **random TR shuffles with no shuffled-brain control**, so the alignment gains are directionally credible but **would not pass our Feghhi/L003 anti-confound protocol**. They only **grow** the model (LoRA + brain head), never compress — so F1 stays open.
- **Causal necessity** — Merlin &amp; Toneva, *ICLR 2026* (arXiv 2603.23091): models trained to be *brain-misaligned at matched perplexity* do substantially worse across 200+ downstream tasks. Brain alignment is load-bearing, causally, not just correlated.
  ...
  > **↳ Correction (R04 full-read, 2026-06-10).** *Original claim:* misalignment at matched perplexity "does substantially worse across 200+ downstream tasks; alignment is load-bearing, causally." *What we did:* full read of arXiv 2603.23091, *"When Language Models Lose Their Mind"* ([`merlin-2026`](../literature/canonical/merlin-2026_when-lms-lose-their-mind.md); text models BERT / GPT-2 / Llama-3.2-1B, gradient-reversal de-alignment). *Corrected:* broadly correct, with one caveat and one understatement. **Caveat:** the damage is **not uniform** (e.g. GPT-2 on Harry-Potter fMRI is only marginal, p=0.055, n.s.). **Understatement:** the **positive arm** shows brain-tuning *up* significantly **helps** downstream — so the paper supports **A3-in-principle (induced alignment buys utility)** more strongly than "misalignment hurts" alone. All at constant parameter count, never under compression — it motivates F1's payoff without occupying its cell.
- **Auditory CNNs** — Freteault et al., *Imaging Neuroscience 2025*: fMRI fine-tuning of a 2.5M-param audio CNN improves 12/19 HEAR tasks, *especially under low data*. Direct support for the sample-efficiency mechanism.
  ...
  > **↳ Correction (R04 full-read, 2026-06-10).** *Original claim:* "improves 12/19 HEAR tasks, especially under low data — direct support for the sample-efficiency mechanism." *What we did:* full read of the PMC version ([`freteault-2025`](../literature/canonical/freteault-2025_auditory-ann-fmri-finetuning.md)). *Corrected:* **12/19** (Wilcoxon p<0.05) is exact. But "especially under low data" is a **directional pattern** (strongest on the ~2-min tasks) with **no formal interaction test and no matched non-brain-tuning control**, so it does not isolate the brain signal as the cause — it is **suggestive, not a clean mechanistic precedent**. This is why R04 re-weights F2 below F1: the contrastive "helps most when text data is scarce" test still has to be run with the control these papers skipped.
...
- **Q0 — Does the signal exist beyond confounds, on real data? (tests A2; the charter's gating question.)** The synthetic pilot (E001) could *not* answer this by construction (L004). **E002** (this session) runs the real encoding-feasibility probe on Tuckute 2024: does an LM's contextual middle-layer representation predict LH-language-network BOLD *beyond* nuisance (length, position, static embeddings), under contiguous splits, vs an untrained-network control? **Kill:** if trained unique $R^2 \approx 0$ or $\approx$ untrained, the premise fails and the thesis reframes to measurement rigor. *(Result: see `experiments/E002_*.md`.)*
- **Q2 — Is the signal a *lever* we can move?** Brain-tune a small model (GPT-2 / Qwen-0.5B) on Tuckute/LeBel and verify the unique $R^2$ *moves* in the expected direction. Establishes that $\mathcal{L}_{\text{brain}}$ is trainable and controllable before we ask it to do anything useful. **Kill:** if alignment cannot be increased by direct optimization, no downstream framing is viable.
- **Q4 — Does preserved/induced alignment buy something? (tests A3 — the untested assumption.)** Compare a brain-regularized small model vs a matched baseline on OOD generalization and/or low-data sample-efficiency (this is F2). **Kill:** if gains stay confined to the alignment metric and never reach a downstream task, A3 fails and the practical claim dies.
- **Q1 — Does perplexity-only KD preserve or destroy alignment? (R04 §6a; the cheap kill-test that gates F1.)** Measure where ordinary perplexity/logit KD lands a student on the alignment axis vs its teacher, before building any $\mathcal{L}_{\text{brain}}$. **Kill:** if KD preserves alignment by default, F1 collapses the way `oota-2026` complicates quantization. *(Result, E003, 2026-06-10: **PARTIAL headroom.** A monotone alignment gradient — conventional gpt2 ≈ teacher > warm-KD > distilgpt2 > from-scratch cold-KD ≈ floor — rules out preserve-for-free, but the "sheds alignment *beyond* the perplexity it costs" claim is unresolved: alignment co-varies with LM quality (r=−0.88 on log-ppl) and the KD-specific dissociation is only p≈0.1 at this ROI-coarse benchmark (L011). F1 is neither killed nor confirmed; see `experiments/E003_*.md`.)*
- **Q3 — The distillation use-case (F1).** Alignment-guided KD vs perplexity-only KD vs structure-aware KD (MiniLM/MGSKD), matched compression ratio and budget, ≥3 seeds, reported as a trade-off curve with confound-corrected alignment. This is the headline thesis experiment; the E001 harness already implements the distillation + anti-confound machinery. **E003 sharpens the protocol: the baseline must be matched on *perplexity*, not just budget**, or an alignment gain cannot be attributed to the brain objective rather than to LM quality (L011).
- **Q5 — The fMRI-free method (F3).** Replace the live-fMRI loss with an abstraction/LID/CKA surrogate validated against real fMRI at Q0–Q2; show it recovers most of the Q3 benefit without the data bottleneck. Stretch goal.

*Note: rungs use the canonical Qn labels (climb order Q0→Q5; see [`../map.md`](../map.md)); they appear here in the original conceptual grouping, not climb order.*
