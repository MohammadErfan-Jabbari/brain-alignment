...
2. **T1.3 — surprisal-residualized cognitive-signal training (the keystone). RESOLVED → INCONCLUSIVE at n=3 (S22).** Full chain ran (G0/G1 PASS → G2v1 metric failed → metric revised → G2v2/v3 PASS → 5+1 arms, twice, on a clean harness after Codex caught a real bug). **Trustworthy clean-v3 verdict:** (a) surprisal-orthogonality is **NULL** (raw≈residual, +4.3 [−3.9,+12.6]) — the specific novelty buys nothing; (b) **generic word-aligned aux-regularization dominates** (~450 AULC gap, E004 `frozen` at scale); (c) the cognition question is **underpowered/unresolved** — residual is best on the mean but CIs touch 0 (n=3), and the log-freq exclusion control is confounded by learnability. The v2 "triple-dissociation positive" was a harness-bug artifact. **The clean main-track positive did NOT materialize; the clean negative is only the *narrow* one (surprisal-orthogonality null).** Resolving the broader question needs more seeds + a learnability-matched control — Erfan's call. → E021. The lit-gate's decisive finding: reading-times/N400 are *extremely well predicted by LM surprisal*, so naive regularization is **circular** (= the fMRI-null mechanism in a new coat; BabyLM showed cognitive aux-objectives ≈ randomized-order nulls). The **only** viable positive mechanism = the **residual of the cognitive signal after the model's own surprisal is regressed out** (the Merlin-Toneva "beyond-next-word" move applied to behavior/EEG). EEG-*guided training* (vs decoding) is genuinely unscooped; behavioral is partly scooped (Deng ACL'24: raw gaze, BERT, in-dist GLUE, no residualization/matched-ppl). **Why GO on a weak prior:** cheap (small decoder LM + ZuCo), clean predeclared kill criterion, and **dual-publishable** — a null generalizes our fMRI negative across fMRI+behavioral+EEG (stronger, main-track-shaped), a surviving residual arm is the new positive. → **E021** (designed next, oracle-gated before compute). Dataset: **ZuCo/ZuCo 2.0** (simultaneous EEG+eye-tracking+reading, public, word-aligned) + GECO/Provo/Dundee for behavioral scale.
