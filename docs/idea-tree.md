...
2. **T1.3 — surprisal-residualized cognitive-signal training (the keystone). RESOLVED → NEGATIVE (S22).** G0/G1 PASS (residual reliable) → G2v1 metric failed → G2v2 (frozen-probe) PASS → 5 arms fired the triple dissociation **but the cognition reading is REFUTED** (panels + Codex): raw≈residual ⇒ surprisal-orthogonality null (DPI-mandated: residual can't exceed raw); effect is generic word-aligned regularization (E004 `frozen` at scale); Codex found a pad-label bug → numbers need a clean rerun but the verdict is theory-robust. **Outcome: a cross-modal NEGATIVE that strengthens Path A; the main-track positive did NOT materialize.** The one mechanically-viable main-track path is now closed. → E021; cheap log-freq confirming control pending Erfan. The lit-gate's decisive finding: reading-times/N400 are *extremely well predicted by LM surprisal*, so naive regularization is **circular** (= the fMRI-null mechanism in a new coat; BabyLM showed cognitive aux-objectives ≈ randomized-order nulls). The **only** viable positive mechanism = the **residual of the cognitive signal after the model's own surprisal is regressed out** (the Merlin-Toneva "beyond-next-word" move applied to behavior/EEG). EEG-*guided training* (vs decoding) is genuinely unscooped; behavioral is partly scooped (Deng ACL'24: raw gaze, BERT, in-dist GLUE, no residualization/matched-ppl). **Why GO on a weak prior:** cheap (small decoder LM + ZuCo), clean predeclared kill criterion, and **dual-publishable** — a null generalizes our fMRI negative across fMRI+behavioral+EEG (stronger, main-track-shaped), a surviving residual arm is the new positive. → **E021** (designed next, oracle-gated before compute). Dataset: **ZuCo/ZuCo 2.0** (simultaneous EEG+eye-tracking+reading, public, word-aligned) + GECO/Provo/Dundee for behavioral scale.
