...
2. **T1.3 — non-fMRI (EEG/MEG/behavioral) brain-signal-guided training — the lead Path-B bet (the only top-main-track lottery ticket).** The one node where a new positive is *mechanically possible*: it changes the coupling (faster dynamics, different SNR/confounds, cheap+abundant behavioral signals like reading-times/eye-tracking/N400), not the metric on a frozen representation. **Gate: a focused lit check** — is it an unscooped gap, and is there a viable positive *mechanism* (Guo'24 EEG→robustness is partial prior art with soft controls)? Only design+run if the gap + mechanism hold. *Lit check launching now.*
