...
Audited the corpus: a narrative workflow (6 agents) + an empirical mining of all **266 subagent runs / 27 sessions** + the precedent-engine reference session. Proposal: [`docs/references/agent-fleet-redesign.md`](../references/agent-fleet-redesign.md). Implemented:
...
- **[confound-catalog.md](../references/confound-catalog.md)** (one source of truth) + **/precheck** (pre-compute gate). (e9b3067)
...
- **.mcp.json deferred** — no Exa transport config findable under `~/.claude` (it's injected by the global harness); a guessed one risks shadowing the working Exa. Needs the real connection details (then a 1-file add). Recorded in [`agent-fleet-redesign.md`](../references/agent-fleet-redesign.md).
