...
| Top-tier positive paper claim. | TRIBE beats the prepared sentence-local textfeat control across six paired seeds, and the raw-row audit reproduced the margin. | Conditional, materially strengthened. | The next burden is not another blind seed extension; choose the estimand: real-brain evaluation with saved seed `3-5` artifacts, stronger context/on-policy non-brain control, or a deliberately narrowed synthetic-target paper. The estimand split is recorded in [`top-venue-on-policy-context-distillation-audit-2026-07-03.md`](top-venue-on-policy-context-distillation-audit-2026-07-03.md). |
...
The seed-extended comparator has now passed a local raw-row `/interpret` audit. The next legitimate state transition is one of:

1. **Real-brain evaluation path:** score saved seed `3-5` TRIBE/textfeat students on Tuckute with the prepared evaluator, treating output as diagnostic and not a verdict.
2. **Stronger non-brain-control path:** build/run contextfeat or design on-policy control only after naming the estimand.
3. **Narrow-claim path:** frame the contribution as synthetic brain-like target survival beyond sentence-local text features, without claiming real-brain specificity.
