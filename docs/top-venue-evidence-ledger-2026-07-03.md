...
| Brain-specific positive branch: TRIBE beats matched-information non-brain target. | Full `textfeat` control completed with `science_ready=true`; `phase3_full_tribe_vs_textfeat_comparison.json` routes to `tribe_stronger_than_textfeat_needs_review`. TRIBE-minus-textfeat gain is `+0.076731` versus KD-only and `+0.069492` versus permuted-control gains. | Promising review branch, not verdict. | Requires `/interpret`, stat/code review, seed-margin audit, and likely extra seeds or a second evidence target. It clears only sentence-local frozen-LM hidden-state supervision, not long-context/on-policy distillation or real-brain evaluation. |
...
| Top-tier positive paper claim. | TRIBE now beats the prepared sentence-local textfeat control in the ready comparator, but the comparator itself routes to review rather than clearance. | Conditional, strengthened. | Requires `/interpret`, stat/code review, likely extra seeds, and a decision between context/on-policy control, real-brain evaluation, or deliberately narrowed claim. The estimand split is recorded in [`top-venue-on-policy-context-distillation-audit-2026-07-03.md`](top-venue-on-policy-context-distillation-audit-2026-07-03.md). |
...
Do not start another heavy run while the ready comparator is unaudited. The next legitimate state transition is one of:

1. **Comparator audit:** switch to `/interpret`; audit seed-aligned TRIBE-minus-textfeat margins, PPL matching, analyzer gates, and code/stat assumptions.
2. **Post-positive burden choice:** inspect the privileged-signal adjacency audit and choose extra seeds, context/on-policy control, real-brain evaluation, or a narrowed claim.
3. **If review breaks the margin:** route toward controlled-negative/protocol framing instead of a brain-specific positive.
4. **If review holds:** prepare the next prechecked evidence step before launching more compute.
