...
| Generic privileged-signal/context/gaze supervision novelty is closed. | [`top-venue-privileged-signal-adjacency-audit-2026-07-03.md`](top-venue-privileged-signal-adjacency-audit-2026-07-03.md), plus canonical notes for Deng 2024 and E024. | Scout-supported frontier claim. | Do not claim "PI/cognitive/gaze signals help models" as the contribution; use it only to motivate stricter controls. |
...
| Brain-specific positive branch: TRIBE beats matched-information non-brain target. | Text-feature control launcher prepared, not run. The control is sentence-local frozen-LM hidden-state supervision, projected to the same target dimension. | Not yet known. | Run the prepared text-feature control only after a positive TRIBE result and free resources. Needs same analyzer gate and preferably more seeds or a second evaluation target. |
| Top-tier positive paper claim. | No current evidence yet. | Conditional. | Requires E016 positive at matched PPL, textfeat control passed, paired effects stable, code/stat audit clean, and likely extra seeds plus either long-context/on-policy distillation control, real-brain evaluation, or a narrowed claim. |
