...
| Saved-student real-brain alignment follow-up. | [`e016_eval_saved_student_alignment.py`](../scripts/e016_eval_saved_student_alignment.py) scored all 18 saved seed `3-5` TRIBE/textfeat artifacts on Tuckute; paired analysis is `phase3_extra_tribe_vs_textfeat_s3-5_tuckute_alignment_analysis.json`. TRIBE target versus KD-only mean `-0.000735`; TRIBE target versus permuted mean `-0.001717`; TRIBE-minus-textfeat gain versus KD-only mean `-0.000862`. | Diagnostic completed; warning for the positive branch. | Still needs `/interpret`, code/stat audit, and claim-scope decision. It weakens the real-brain-positive route but is not a final verdict because it is post-hoc, ROI-level, and only covers saved seed `3-5`. |
...
1. **Diagnostic interpretation path:** run `/interpret`/code/stat review on the Tuckute diagnostic before treating it as a result.
2. **Narrow-claim path:** frame the contribution as synthetic brain-like target survival beyond sentence-local text features, without claiming real-brain specificity.
3. **Robustness/rerun path:** predeclare what real-brain endpoint or saved-student rerun would be capable of overturning the Tuckute warning.
