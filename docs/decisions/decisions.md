...
**Retired.** The snapshot hook and `.claude/state/` were deleted in `e4c098e` (2026-07-13); the auditor fan-out and the report layer went with D056. See D068. The record below describes the original design.
...
| `honesty_writecheck`, `prose_writecheck` | RETIRED by D068 (was: KEEP, narrow) | Deterministic manuscript provenance/register floors; remove report and retired-pipeline dependencies |
...
## D068 — Hooks are surgical gates; content verification is agent work — 2026-08-31 (/meta, Erfan-approved)

**Decision.** A hook in this repo is legitimate only if it decides yes or no **from the intercepted call's own arguments** — no repo path, no file contents, no taste — and the action it stops is irreversible or expensive enough that a later reviewer arrives too late. Anything that must open a file to judge it is a verification process and belongs to a skill or a read-only subagent, never to a hook.

**Applied.**

+ **Retired** `honesty_writecheck.py` and `prose_writecheck.py`. Both guarded `docs/manuscript/extended/` and `docs/manuscript/public/`, deleted on 2026-08-25 in `64d4332`, so both had been silent no-ops for six days while D044 and D047 continued to describe them as *the* load-bearing fabrication guard ("the one check the unlogged-burst bypass cannot skip"). Repointing them at `rewrite/` was rejected: that works only until `rewrite/` stops being canonical, which is the same clock that already ran out once.
+ **Hardened** `agent_routing_lint.py`: fable now returns `permissionDecision: deny` instead of a stderr warning that exited 0. A ban that only warns is not a ban, and every fable spawn had passed since the hook was written. The opus preference for judgment-heavy agents stays a warning, because it is a preference and a wrong deny breaks a legitimate spawn. A missing model is left to the global `require-subagent-model.sh`, which denies it.
+ **Added** `bash_gate.py`, two denials on `PreToolUse` / `Bash`. (a) An unsandboxed `codex exec`: bubblewrap cannot initialise in this container (L033), so the sandboxed form returns values the model guessed, which are indistinguishable from a real answer, and a `-c key=val` override has silently reverted the explicit flag once. (b) Irreversible Git: hard reset, forced clean, forced push (but not `--force-with-lease`), amend, forced branch delete, and stash drop or clear. Self-checked by `tests/test_bash_gate.py`, 30 cases.
+ **Retired** `section_swarm.js` (748 lines). It was owner-authored and owner-driven across five sessions, but **no decision ever authorized building it** — D067 licenses the parent's autonomous-apply behaviour, not the apparatus. Its last four launches all ended in fail-loud infrastructure stops and Erfan stopped it on 2026-08-31. [`references/section-review-swarm.md`](../references/section-review-swarm.md) is retained as a process record of what it did and what its routing failures taught; the code is recoverable from Git history. Judgment returns to the `/review` panel.
+ **Removed** `.claude/state/` (no writer since `e4c098e`, no reader ever) and the `graphify`, `scientific-writing`, and `sci-write-v2` skill trees (0 tracked files, 0 real files, 380 KB of bytecode for sources deleted in `e4c098e`).
+ **Fixed** `manuscript_check.py`. `source_files()` globbed only inside the target directory, so the submission twin's `\input{../rewrite/numbers}` was never read and it reported **270 false "undefined keyed number" findings**; declaration scope is now the `\input`/`\include` closure of the target plus the enclosing `main*.tex`, which also makes single-file invocation correct for the first time. LaTeX dimensions and TikZ coordinates are stripped before the bare-number check (widths, `em` skips, and node coordinates were reading as unsourced results). Vendored `style/` is out of check scope. The default target moved from the deleted `extended/` to `rewrite/`. The submission tree now reports 0 findings, all 270 having been false; `rewrite/` still passes on 16 sources.

**Why.** The repo derived this axis on 2026-06-19 as **L052** — a prose-defect class splits on whether a banned token betrays it, and that axis dictates the tool — then filed it in `learnings.md`, which no instruction file loads, so the tooling kept violating a rule the repo had already learned. The four hooks shipped to date each failed differently, and the pattern is the rule: the two that read a repo path died when the path moved; the one Erfan calls good reads `tool_input.model`; the `Stop` hook that re-injected intent rather than checking an argument cost roughly $1380 across 46 files past his own "wrap up" (L031). The decisive property of a content-reading hook is that once it stops firing it is indistinguishable from one that is passing.

**Supersedes.** The hook layer of D044, D046, and D047 (the deterministic write-time honesty and prose floors, and the `Stop`-hook terminator; D047's prescription directly contradicts L031 and is not revived). The D056 fleet-audit row keeping the two writechecks. The D038 `wrap_session_snapshot.py` hook and its `.claude/state/wrap/start.json`, both already gone in `e4c098e` but still described as live. **Overrides D039's scope** with Erfan's explicit approval this session: D039 declined a staging-hygiene guard, which is fully recoverable, and that reasoning does not extend to unrecoverable loss.

**Owed.** Retiring the deterministic prose and provenance floors leaves `/write` step 5 plus one fresh independent review as the only guard, and a session can skip both. The replacement is a read-only verification layer (`evidence-number-auditor`, `citation-support-auditor`, `manuscript-twin-auditor`) that compares manuscript values against their owning E records at the record's displayed precision. That is the check `manuscript_check.py` structurally cannot make: its `resolves()` confirms an E record **exists** and never opens it, so `\evd{E008}` on a sentence E008 contradicts passes cleanly. Until that layer lands, the manuscript has no automated provenance check beyond marker resolution, and this is the one regression this decision knowingly accepts. **Closed by D072 (2026-08-31):** all three agents exist, are read-only by construction, and are wired into the writing protocol at step 14. The regression stood for one day.
...
| The maintained question tree itself | `docs/manuscript/rewrite/AGENTS.md` (moved to the sibling `question-tree.md` by D071; the layer and its owner folder are unchanged) |
...
## D071 — Every rule has exactly one owner, and the question tree is not a folder contract — 2026-08-31 (/meta, Erfan-directed)

**Decision.** Finish the instruction-file consolidation. Three parts.

**(1) The question tree moves out of the folder contract.** `docs/manuscript/rewrite/AGENTS.md` was 521 lines, of which 424 were the tree. A folder `AGENTS.md` is auto-loaded by every harness that touches the folder, so the tree was about to become a fixed tax on every manuscript edit once that folder gained its `CLAUDE.md` symlink. The tree now lives in `docs/manuscript/rewrite/question-tree.md`, a plain document read when the argument's structure is in play; the contract drops to 108 lines and points at it. This changes the file, not the ownership: the tree is still the single source for the leaf questions, still updated in the same change as the prose, and every one of the six references to it was repointed in the same commit.

**(2) Two authorities gained the folder contract they never had.** `docs/experiments/` is the first of the four authorities and had no contract at all; `docs/timeline/` had 53 files, two incompatible filename conventions, and no written rule for when an entry is even warranted. Both now have one, with the traps that are specific to them: for experiments, that an `ENNN` is permanent and a gap in the sequence is a decision rather than a missing file (`E007` and `E027` were deliberately never built), that predeclaration precedes running, and that seeds are not biological inference units; for timeline, that it is not an authority, that the date-only filename is the current form while existing names stay as they are, and that not writing an entry is preferred over writing a thin one.

**(3) The rules mined from the session corpus were placed, one owner each.** Fourteen rules that Erfan had repeated to agents but that appeared in no instruction file, plus two found while executing. Placement: the root contract took the general ones (produce nothing unasked, terse answers, the three-rung autonomy ladder, and that a defense-prep answer is advocacy and never a source of a number); `.claude/AGENTS.md` gained a dispatch contract that is the standing form of what Erfan retyped into nearly every review task (read-only reviewers, never merge first-round output, explicit `model:` and `effort:`, gate degenerate output, never adjudicate with a tool's own `--selftest`, smallest diff wins); the manuscript contract took the drafting order, the `numbers.tex` owner-confirm rule, and the banned-token list; the `teach` skill took the rule that restating the source is not an explanation.

**The banned-token list has a home and a mirror.** Erfan banned four things by name across sessions: the em dash, *kill-gated*, *standardly*, and *prose* and *verdict* as generic nouns. Only the em dash was enforced, and no instruction file stated any of them. The list now lives in `docs/manuscript/AGENTS.md` and is mirrored into `prose_lint.py` as a new hard-failing `OWNER_BANNED` list, with `kill-gated` promoted out of the soft register warnings. All four are absent from both manuscript trees, so the gate passes on arrival, and the linter's self-test still passes. The rule attached to it is the [L052](../learnings.md) split stated plainly for the first time: a closed token list is complete by construction and belongs in a script, while register and taste are not lintable and belong to a fresh reviewer.

**Also cleaned, under the same one-owner rule.** `docs/AGENTS.md` restated 35 of root's lines and is now the folder's map plus the five rules that are genuinely local. `docs/literature/AGENTS.md` lost a paper-role table it described in its own text as the old index, and a fresh-search to-do list. `docs/learning/AGENTS.md` no longer calls an empty ledger the single source of resume truth while resuming from lessons. `scripts/AGENTS.md` handed the toy-pilot narrative, estimand rule, and run commands back to `E001` and kept the file map. Two vault files gained the frontmatter the Obsidian convention requires. `docs/AGENTS.md` also carries the rule that would have prevented [L077](../learnings.md): a retired mechanism is removed from every file asserting it live, in the commit that retires it.

**Found, not fixed, and reported instead.** The `submission/` tree's LaTeX build now fails. `latexmkrc` pins `$biber = '/usr/bin/biber'`, which is 2.19, and biber 2.19 refuses the relative parent path in `\addbibresource{../rewrite/references.bib}`; the biber on `PATH` is 2.17 and accepts it. Nothing in this change set touches that tree's source. Changing a deliberate build pin on a supervisor-facing derivative is Erfan's call, and the canonical `rewrite/` tree passes, so the scientific authority is unaffected.

**Reverses if:** a rule turns out to need two owners, which would mean the layer boundary is drawn in the wrong place. Fix the boundary, not by copying the rule.

## D072 — The verification layer lands; the corpus improves and no agent edits its own criteria — 2026-08-31 (/meta, Erfan-directed)

**Decision.** Add the three read-only verification agents D068 owed, the rule corpus they read, and the `session-mining` skill. Close the last two folder symlinks. Fix the submission tree's build. This closes the plan Erfan reviewed.

**(1) The three auditors, and the hole they fill.** Every one of the eleven existing agents is a design gate, a claim attacker, or a literature gatherer. Not one verified that what is written matches what is recorded, and the deterministic gate structurally cannot: `manuscript_check.py`'s `resolves()` confirms that an E record *file exists* and that a key *is declared*, and never opens the record, so `\evd{E008}` on a sentence E008 contradicts passes cleanly. `evidence-number-auditor` resolves each `\result{key}` through its `% [ENNN]` tag, opens the record, and compares numerically at the record's precision, also checking verdict fidelity. `citation-support-auditor` takes the semantic half only, since key-to-bib integrity is already deterministic and clean at 32 keys with zero dangling: it resolves 51 `\autocite` and 7 `\textcite` to the 69 canonical notes and judges the support relation. `manuscript-twin-auditor` is the verifier `manuscript-sync-pending` never had, classifying every divergence as `INTENTIONAL-CUT`, `DRIFT`, or `REVERSE-EDIT`, with a reverse edit always a `FAIL`.

**All three are read-only by construction, not by instruction.** A `tools:` allowlist is a harder guarantee than a paragraph, so none carries `Write` or `Edit`, and none carries `WebSearch` or `WebFetch` because web access is where dispatched children have derailed here. `citation-support-auditor` has no `Bash` either. They declare `model: sonnet, effort: high` explicitly, answering Erfan's 2026-06-20 question about auditors on opus: these are traversal jobs and must be cheap enough to run per subsection on every pass.

**(2) Self-improving, not self-editing.** Erfan asked for dynamic, self-improving agents rather than static scripts. The corpus is where that lives, and the boundary is deliberate: an agent that can edit its own criteria has a direct incentive to weaken them, because loosening a tolerance makes its own `PASS` cheaper, and that is exactly what `.claude/AGENTS.md` forbids with no recorded reason. So an auditor emits `PROPOSED-RULE: <class> | EVIDENCE: <file:line> | WOULD-HAVE-CAUGHT: <miss>`, the parent evaluates it, and the parent appends it to `docs/references/manuscript-verification-rules.md`. That file is monotonic: it may only add checks. A proposal that would relax an existing one routes to `/meta` with a decisions entry. It opens with sixteen rules, each carrying the evidence that earned it, and follows the shape `confound-catalog.md` already proves.

**(3) Coverage is computed by the parent, never claimed by the child.** The degenerate-child gate from the retired swarm survives in `.claude/AGENTS.md` and at protocol step 15: grep the scope independently, require one `ID:` block per item, auto-reject `RESULT: PASS` with `CHECKED: 0`, treat `INCOMPLETE` as never `PASS`, spot-check two `RECORD: file:line` values against the file, retry once with the reason appended, then surface. A 114-character reply cannot fabricate 105 per-item blocks, which is why every output is per item rather than a summary.

**(4) `session-mining`.** Erfan wrote this method himself and dispatched it five times verbatim. The root contract said where the transcripts are and warned they are too large to read; it never said how. The skill carries the procedure and the hazard that invalidated a count in this very session: the injected `AGENTS.md` preamble rides inside a user-role message, so a naive frequency count measures the instructions rather than the behaviour ([L078](../learnings.md)).

**(5) Symlinks: `CLAUDE.md → AGENTS.md`, and not `@AGENTS.md`.** An agent in another session proposed filling `CLAUDE.md` with an `@AGENTS.md` import instead. Both were tested headlessly here. The import works: a probe answered from the imported contract. But when the target is renamed, the import fails **silently** — the same probe returned `UNKNOWN` with no warning, which is precisely the [L077](../learnings.md) failure mode this repo already paid for. A broken symlink is visible to `ls`, to `[ -e ]`, and to the check loop in one command. The decisive reason is the write-through refusal already verified in this session: `Edit` and `Write` refuse to write through a symlink and name the target, whereas with an import an agent told to update `CLAUDE.md` edits an eleven-byte stub and either destroys the import or creates a second owner, which is the drift this entire overhaul removed. The import's real advantages, Claude-only additions alongside the shared contract and working on filesystems without symlinks, do not apply: this is one Linux node, and Claude-only apparatus already has `.claude/AGENTS.md`. Symlinks stay, now on all fifteen folders that hold an `AGENTS.md`.

**(6) The submission build, fixed at the path rather than the pin.** `latexmkrc` pins `$biber = '/usr/bin/biber'` and biber 2.19 refuses the relative parent path in `\addbibresource{../rewrite/references.bib}`. The obvious fix, repointing to the biber on `PATH`, is wrong: that one is 2.17 and cannot validate this tree's BCF 3.10 control file, verified with `--validate-control`. So the pin is correct and the path was the defect. `submission/references.bib` is now a symlink to the canonical `rewrite/references.bib` and the `\addbibresource` line is local, which keeps one source of references and works under any biber. `manuscript-check` returns to PASS on 14 sources.

**Verified:** both trees PASS, `bash_gate` 34/34, `prose_lint --selftest` PASS, 16 skills and 14 agents load with valid frontmatter, the three auditors carry no write or web tool, 159 relative links resolve, 15 `CLAUDE.md` symlinks valid with none missing.

**Reverses if:** the auditors prove too expensive to run per subsection, in which case they move to subsection close and section close rather than every pass. If an auditor is ever given `Write`, this decision is void.
