...
- **Where things live:** canonical status → `ladder.md`; **current-truth findings → the finding-reports `reports/R*.md`, one claim each, Q-tagged (D036)** — Q3 = `R06` (the per-individual null + averaging confound), more added as the analysis week covers each finding; the teaching narrative of the whole arc → `reports/R05`; the analysis-week plan → `analysis-roadmap.md`; experiments → `experiments/E*.md`; decisions/lessons → `decisions/decisions.md` + `learnings.md`.
- **Report convention (D036):** a report owns one claim, tagged with the question it answers, states current truth only (no "obsolete / not yet narrated" scaffolding — history lives here in `map.md` + `timeline/` + `decisions/`), and follows the skeleton header→question→design→evidence→verdict→caveats. Spec in the `scientific-writing` skill.
