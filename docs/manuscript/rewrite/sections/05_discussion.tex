...
\subsection{Failure localization across the evidence chain}
...
The recorded-target branch then lost support at the intervention stage: none of the valid inference units established a participant-general advantage from correct stimulus--response pairing \evd{E008}\evd{E011}\evd{E013}\evd{E017}.
...
\subsection{Relation to positive prior studies}
...
A positive result can be valid for any one of these estimands without satisfying all later stages.
...
Whether a positive prior result would survive every additional stage is an open empirical question, not a conclusion licensed by the present experiments.

\subsection{Implications for future brain-guided training}
...
The pre-training test should require both practically measurable unique signal beyond nuisance and adjudicated target specificity relative to that control.
...
The failure stage should determine whether a study continues, redesigns, or stops.
...
When a practically meaningful effect is excluded or an upstream stage fails, the warranted choices are to stop or redesign the target, control, or assay rather than accumulate more target-fitting evidence.
