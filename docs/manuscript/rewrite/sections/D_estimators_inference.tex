...
The exact synthetic-target measurability assay has one other declared exception: it reuses target-standardization moments fixed during target construction, while its target \ac{pca} and readouts remain fold-local. \evd{E006}\evd{E013}\evd{E017}\evd{E030}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{30mm}>{\raggedright\arraybackslash}p{32mm}>{\raggedright\arraybackslash}p{52mm}>{\raggedright\arraybackslash}p{34mm}@{}}
...
\renewcommand{\arraystretch}{1}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{30mm}>{\raggedright\arraybackslash}p{52mm}>{\raggedright\arraybackslash}p{64mm}@{}}
  ...
  Comparator adequacy is a design condition for attribution; statistical precision cannot repair a non-identifying comparator \evd{E026}\evd{E030} \\
  \midrule
  Saved-student biological-transfer assay &
  ...
  \midrule
  Bounded practical-utility evaluation &
  Eight paired technical-seed contrasts and an observed-variance 80\%-power \ac{mde} &
  The conclusion remains conditional on the tested corpora, models, and procedure; task families are not biological repetitions \evd{E009} \\
\end{longtable}
\endgroup
...
The \result{e030_practical_threshold} unique-\(R^2\) rule was inherited prospectively to decide whether participant-level biological transfer justified a matched-control follow-up, then frozen as a reference for the exact synthetic-target measurability assay. \evd{E025}\evd{E030}
