---
title: "Maintained Question Tree"
tags: [manuscript, rewrite, question-led-writing]
aliases: [question-tree, rewrite-question-tree]
---

# Maintained Question Tree

This file is the maintained question tree for the canonical thesis manuscript, and the only copy of it. Every prose paragraph in [`main-rewrite.tex`](main-rewrite.tex) and its inputs answers one leaf question here.

It is deliberately a plain document rather than a folder contract, so that editing a manuscript file does not load the whole tree into context unasked. Read it when the argument's structure is in play: when planning a subsection, when a paragraph has no leaf to answer, or when a result changes what the reader must be told. The method that operates on it is the [`question-led-writing`](../../../.claude/skills/question-led-writing/SKILL.md) skill, and the gates are in [`../AGENTS.md`](../AGENTS.md).

Update this tree in the same change as the prose whenever the structure moves. A tree that no longer matches the manuscript is worse than no tree, because the audits read it as the target.
...
2. How do baseline choice at comparable \ac{lm} quality, comparator validity, endpoint choice, training seeds, and inference unit change what a positive contrast establishes?
3. What does this thesis add without reinterpreting or claiming to refute those prior results?

Section 2.3 owns the factual literature map and unresolved pre-results gap. This subsection owns only the post-results compatibility judgment and must cross-reference, not resummarize, the earlier studies. Design differences establish non-equivalence; do not claim that they caused the different outcomes. Answer leaf 2 by cross-reference and consequence, never by restating the catalog: Section 2.4 declares comparator validity and endpoint choice ex ante and Section 7 restates them, so a third statement of the rules here is a defect, but applying them to prior positive studies is this subsection's own job and no other section does it.
...
- [Canonical thesis manuscript contract](AGENTS.md)
- [Manuscript writing protocol and gates](../AGENTS.md)
- [Question-led writing method](../../../.claude/skills/question-led-writing/SKILL.md)
