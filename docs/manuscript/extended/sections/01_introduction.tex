...
This instrument almost always runs in one direction, to \emph{measure} alignment as a property observed after a model is trained.
This thesis asks the inverted question: can that alignment instead be \emph{used} as a training signal, a term added to a model's objective that rewards representations the brain would predict, and does a model trained that way gain anything a model trained on text alone does not.

Inverting the question is only worth a thesis if it clears a hard prior.
...
The methods make this argument formal and tie it to the information-theoretic bounds behind it.

Using fMRI to shape a language model is, by 2026, already done.
...
This work therefore fixes its measurement standard before measuring anything: the reported quantity is never a raw score but the unique variance the model adds on top of a low-level nuisance set, on contiguous splits, with a same-architecture untrained network as the control \autocite{oota2023,merlin2024}.
The program is then a sequence of kill-gated questions, each asked only if the one before it holds: whether the signal is real beyond confounds, whether plain distillation preserves it, whether an alignment-guided objective moves alignment beyond what perplexity already implies, whether any such gain holds per individual at matched perplexity, and whether it improves a downstream task.

This manuscript reports those results in the order their questions depend on one another.
