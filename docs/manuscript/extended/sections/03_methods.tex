...
% Treatment: content kept; Round 3 = FLOW PASS (esp. 3.2: break the cond-MI sentences).
...
\subsection{The encoding model and the alignment score}
...
We do not report a raw score of this kind on its own, because a model can predict the brain for reasons that have nothing to do with language understanding: utterance length, word position, generic word identity.
We therefore report the unique variance the model adds \emph{on top of} a low-level nuisance set $Z_{\mathrm{nuis}}$, the difference of two cross-validated fits,
\begin{equation}
...
The score is a lower bound on the information the model's representation carries about brain activity beyond the low-level confounds.
...
The regime where a brain signal could matter follows from an information budget, and that budget predicts where the experiments should and should not find an effect.
...
A weak-but-aimed brain prior should therefore help most in low-information regimes (small, under-trained, or aggressively compressed models) and wash out where the text signal already dominates, which is the regime logic behind targeting distillation.

\subsection{The verdict statistic: the trained\textendash untrained gap}

We report not a trained model's absolute unique-variance score but the \emph{gap} between it and a same-architecture untrained network fit under identical nuisance and splits.
...
\subsection{Distillation setup and the brain training term}
% [NEW in this scope] Required because Results now reach Q1 (plain KD) and Q2 (the lever).
% Round-2 goal + Round-3 prose to come. Beats to cover:
%  - knowledge distillation: teacher -> smaller student, the matched-budget definition
%    (compute-matched, and the step-count caveat from E003);
%  - the brain training term \Lbrain added to the objective, the co-trained MSE form (D010),
%    and the mixing weight \lambda; the cold/warm-init arms and the labelled drift controls;
%  - the permuted-twin specificity control (brain-\emph{specificity}, not reality), reused from §3.5;
%  - what is held fixed across arms (fixed verdict layer within one architecture).
\gap{Round-3: write the distillation + \Lbrain{} training setup. The mechanism it describes is the bottom (``use'') stage of Figure~\ref{fig:pipeline}; cross-reference rather than redraw.}
