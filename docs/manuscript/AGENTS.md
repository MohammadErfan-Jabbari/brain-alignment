...
After changing prose, recheck the whole active section against the terminology and acronym contracts rather than validating only the edited sentence. A statistical relabel is not finished at the sentence level: sweep math symbols and their superscripts, table headers, figure captions, and the `% [Ennn]` comments in `numbers.tex`. Relabelling eleven sentences in Section 4 left four `\mathrm{CI}^{\mathrm{family}}` symbols behind, one of them two lines above a sentence that contradicted it. When two results are adjudicated against the same threshold or comparator, they carry the same disclosure set; reporting a minimum detectable effect for one exclusion and not its weaker sibling makes the silence read as strength. Split a paragraph when it serves distinct reader questions, such as reporting findings and stating contributions, and synchronize the maintained question tree when that changes the argument's structure.
