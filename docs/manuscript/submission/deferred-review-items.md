...
- "auxiliary-target family" (§3.4, recovery assay) is used with no definition anywhere, and the appendices do not define it either (verified 2026-08-28 by the round-2 judge); needs an owner decision on where to define it.
- "scaled synthetic-target family" / "each scaled synthetic-target arm" (§3.5 quality rule): the correct gloss depends on whether the block-permuted sensitivity arms are inside the family (owner confirmation needed). Verified facts: no loss-weight sweep exists (λ_target = 0.1 fixed in Appendix C); E016's "scaled" refers to the 95,999-sentence corpus scale. If confirmed, candidate gloss: "the synthetic-response arms trained at the 95,999-sentence corpus scale".
- Utility-row endpoint naming (Table `tab:evidence-map`): "held-out language or task endpoint" flagged by judge 1 (2026-08-28) as a possible coinage; candidate for the repo-wide naming pass.
- Recorded-response vs permuted-response WikiText quality match: no tolerance is declared anywhere (§3.5 says only "their declared approximate quality match", and neither appendix specifies one; Results 04 says "no gross quality imbalance without establishing a formal equivalence margin"). Needs an owner decision: declare the tolerance or state explicitly that no margin exists.
