...
Four adjacent literatures bear on the question, each studying something different: brain-encoding studies \autocite{oota2023,merlin2024}, brain-response-guided optimization \autocite{moussa2025,negi2025,merlinData2026,vaidya2026}, privileged-information and feature-distillation work \autocite{lopezpaz2016,saadi2025}, and model-compression studies \autocite{oota2026}.
...
Together, these controls restrict the interpretation of a positive score to held-out predictive value added by learned \ac{lm} representations under the declared assay: the stated combination of stimuli, response definition, readout, and controls.
...
Recorded \ac{fmri} responses vary with the participant and session beyond what the text fixes, whereas synthetic brain responses generated from text carry only the structure learned by a fixed generator, a model trained beforehand to map text to brain responses.
...
It may nevertheless present generator-learned structure in a form that is easier for the student to learn or recover, a hypothesis motivated by privileged-information and feature-distillation results rather than a consequence of the target's deterministic construction \autocite{lopezpaz2016,saadi2025}.
...
Manipulation is therefore checked in two parts, both against the ordinary \ac{kd} arm.
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{24mm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
    ...
    No permuted or matched text-derived target, smaller student, or ordinary \ac{kd} comparator. \\
...
Text-model interventions report gains on multilingual, probe, commonsense, and reasoning endpoints \autocite{negi2025,bilgin2026,merlinData2026,xiao2026}.
...
However, these studies differ in model, training target, endpoint, personalization, and transfer setting, so their positive results do not establish the incremental value of brain responses for a fixed smaller student.
...
The vision study in Table~\ref{tab:related-designs} is the case in point \autocite{pirlot2022}.
...
Measurement-validity research adds requirements for evaluating the student after training.
...
The two routes then carry different claims: pairing-specific attribution and utility in the recorded-response route, and manipulation, transfer, and attribution in the synthetic-response route, alongside an independent pathway qualification.
...
\begin{figure}[tbp]
  ...
  \begin{tikzpicture}[
    ...
    alabel/.style={font=\tiny, text=black!70, inner sep=1.5pt, fill=white}
  ]
    ...
    \node[widebox] (pairing) at (3.2,-3.40) {\textbf{Pairing-specific attribution}\\recorded vs.\ permuted responses, change in controlled brain predictivity}; % \evd{E008}
    \node[widebox] (recordedutility) at (3.2,-6.10) {\textbf{Matched practical endpoint}\\utility after an established change, with paired uncertainty}; % \evd{E009}
    \draw[contextlink] (rmeasure.south) -- node[alabel,right]{context} (rmeasure.south |- pairing.north);
    \draw[contextlink] (rheadroom.south) -- node[alabel,right]{context} (rheadroom.south |- pairing.north);
    \draw[link] (pairing) -- node[alabel,right,align=center]{must pass\\first} (recordedutility);
    ...
    \node[box] (manipulation) at (8.85,-3.40) {\textbf{Target uptake and\\retained-student movement}\\student changed at\\acceptable \ac{lm} quality}; % \evd{E025}
    \node[box] (transfer) at (12.35,-3.40) {\textbf{Direct biological\\transfer}\\fresh \ac{fmri} endpoint\\vs.\ ordinary \ac{kd}}; % \evd{E025}
    \node[box] (attribution) at (8.85,-6.10) {\textbf{Brain-response-specific\\attribution}\\matched text-derived\\control required}; % \evd{E026}
    \node[box] (projection) at (12.35,-6.10) {\textbf{Independent pathway\\qualification}\\50-component linear\\target projection}; % \evd{E030}
    \draw[contextlink] (sheadroom) -- node[alabel,right]{context} (manipulation);
    \draw[contextlink] (smeasure) -- node[alabel,right]{context} (transfer);
    \draw[link] (manipulation) -- node[alabel,above,align=center]{must pass\\first} (transfer);
    \draw[link] (manipulation) -- node[alabel,right,align=center]{must pass\\first} (attribution);
  \end{tikzpicture}
...
These properties include geometry, scale, baseline predictability, how much the target's prediction can still improve, and the early gradient magnitude it exerts.
...
The 50-component linear target-projection assay tests whether the declared projection of the training target adds controlled predictivity of recorded \ac{fmri} responses beyond nuisance and the frozen row-twin control, the same target rows with their sentence pairing broken; it qualifies that linear pathway alone.
