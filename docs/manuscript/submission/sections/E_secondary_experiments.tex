...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    & Records why stopping prevented an uninterpretable downstream attribution claim.
...
The first changed how many participants contributed to the intervention target.
...
After correcting the subset analysis, the apparent improvement did not form a stable monotonic dose--response curve.
The result therefore does not identify a causal benefit of adding more participants to the training target. \evd{E010}

The second analysis held the frozen representation and encoding procedure fixed while averaging increasing numbers of recorded responses.
...
It changes what is being predicted, however, and does not supply participant replication or rescue the individual-response intervention. \evd{E014}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    & The procedure did not establish the required manipulation.\evd{E013}
...
The standard is therefore narrower than reproduction: the question is whether a reported effect appears under the local conditions, not whether a reduced mismatch can confirm or refute the source study.

The first diagnostic adapted the differentiable temporal-head and target-specificity logic of Negi et al. \autocite{negi2025} to the available LeBel responses \autocite{lebel2023} and decoder-only \acp{lm}.
Under the gentle schedule, the retained representations changed little; stronger schedules caused severe language degradation.
...
Neither diagnostic supplied an external positive control for the present intervention pipeline.
They narrow what the local pipeline recovered while leaving the external literature open under its original conditions.
...
Once a clean temporal gap was imposed, the controlled residual was bounded but could not close the ceiling question.
...
Raw and residual reading-time targets initially produced similar frozen-probe learning advantages over simple controls.
...
Natural-reading gaze was measurable but not informative under the tested assay.
...
No corresponding claim was tested for \ac{eeg}. \evd{E024}

Stopping at these failed prerequisites prevents later results from being interpreted as identified intervention effects.
...
An analysis-only slope-and-knee screen did not identify a high-quality flat regime that justified the intervention.
No paired training test was authorized, so the screen contributes no evidence for or against objective-specific shedding. \evd{E023}
...
Its activation criterion was not met, and the saved text-derived auxiliary control was not valid for that role.
...
The route remains on design hold because the exact external protocol depends on unavailable author artifacts; proceeding would require explicit authorization of a clearly labeled reimplementation.
...
It remained contingent on an informative cross-modal regime; only its algebraic consistency check was completed, without independent review or execution. \evd{E029}
