...
\begin{table}[tbp]
  ...
  \caption{Categories of supporting analysis used in this appendix.}
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    & Only the highest tested weight cleared its fold-level interval against the matched permuted control, and it did so at a substantial language-quality cost.\evd{E005}
...
Training a model with an auxiliary head on residualized reading times initially improved a frozen-representation probe on held-out text, and the improvement exceeded every control matched on how learnable its own target was.
...
A screen of the alignment-versus-quality curve, run on recorded values without new training, looked for a region where alignment is nearly flat in language quality, because only there can a difference be attributed to the training objective rather than to quality.
One qualifying pair was found, but only below the language-quality range the design required, so no decisive paired training test was run and the screen contributes no evidence for or against whether ordinary fine-tuning loses brain alignment because of its training objective. \evd{E023}
...
Building it required the preceding saved-student transfer result to meet its predeclared participant-level continuation rule, and that rule was not met.
