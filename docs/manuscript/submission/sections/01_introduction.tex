...
Under \ac{lupi}, a target available only during training can shape which representation the student settles on without changing the inputs available at deployment \autocite{lopezpaz2016,saadi2025}.
...
It depends on the recorded responses: which participants were scanned, which stimuli they read or heard, how their responses were aggregated, and how reliably each was measured.
It depends equally on the data split, the nuisance features (position, length, and static lexical features), the class of readout that maps representations to responses, and the unit over which uncertainty is computed.
...
Encoding studies map which linguistic properties and model layers support prediction of recorded \ac{fmri} responses \autocite{oota2023,merlin2024}.
...
Compression research varies model size without changing the training objective, and reports that brain alignment after pruning or quantization depends on the compression method and model scale \autocite{oota2026}.
Together these studies do not establish whether training with brain responses gives a fixed smaller student an incremental advantage over a matched text-derived auxiliary target, at comparable \ac{lm} quality and with inference taken over participants.

We therefore test that question directly, evaluating each of its component claims against the evidence that claim alone requires.
...
We must also determine whether training improves prediction of independently recorded \ac{fmri} responses and performance on an external task.
A failure at one step limits what the claims that depend on it can establish, and leaves the other claims untouched.

This study established controlled brain predictivity and a student that training measurably changed, but not a brain-specific training benefit.
...
For synthetic brain responses generated from text, the 50-component projection of the training target did not clear the threshold fixed in advance for that linear assay, which required a minimum increment in held-out prediction of recorded \ac{fmri} responses beyond the nuisance model.
...
A post-training readout on the independent Tuckute sentences did not resolve whether the student retained more information about the target than a student trained by ordinary \ac{kd}.
...
The available text-derived auxiliary target was not sufficiently comparable to attribute any difference between the two arms specifically to brain-response content.
