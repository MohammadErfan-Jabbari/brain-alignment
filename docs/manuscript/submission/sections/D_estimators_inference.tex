...
This appendix answers four narrower questions.
They are how the predictive quantities are related, how fitting and aggregation are nested, how uncertainty and decision rules are assigned, and where information-theoretic analogies stop.
Appendix~\ref{app:data-targets} defines the inputs and preprocessing, Appendix~\ref{app:training} defines the training objectives and retained model states, and Section~\ref{sec:results} reports the empirical outcomes.
...
\{1,10,100,1000,10000\}.
\]
Each multi-output fit selects one scalar penalty on its training partition by generalized cross-validation, the closed-form leave-one-out criterion, rather than by an explicit fold split; it does not select a separate penalty for every response or target coordinate.
...
The governing leakage rule is simple.
Every data-adaptive operation that can use stimulus-domain information is fitted on the relevant training complement.
...
The 50-component linear target-projection assay has one other declared exception.
It reuses target-standardization moments fixed during target construction, while its target \ac{pca} and readouts remain fold-local. \evd{E006}\evd{E013}\evd{E017}\evd{E030}

\looseness=-1 Table~\ref{tab:cv-nesting} records the held-out boundary, learned operations, and final inference unit for each scientific role.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{27mm}>{\raggedright\arraybackslash}p{29mm}>{\raggedright\arraybackslash}p{47mm}>{\raggedright\arraybackslash}p{31mm}@{}}
  \toprule
...
Let \(\mathcal C_u\) be the set of technical cells contributing to final unit \(u\), such as its seeds, folds, or seed--fold combinations, let \(d_{u,c}\) be the matched contrast in cell \(c\), and let \(\mathcal A_u\) denote the declared within-unit aggregation.
\begin{equation}
  \widehat\theta_u
  =
  \mathcal A_u\!\left(\{d_{u,c}\}_{c\in\mathcal C_u}\right),
  \qquad
  \widehat\Theta
  =
...
\begin{equation}
  ...
  \label{eq:t-interval}
\end{equation}
Here \(\alpha\) is the two-sided error rate and \(s_\theta\) is the sample standard deviation of the \(U\) final-unit estimates.
Equation~\ref{eq:t-interval} uses the number of final units \(U\), not the number of coordinates, folds, or technical runs.
Where a table reports a descriptive interval over technical seeds, it applies the same formula to the \(K\) matched seed contrasts below and is read as a stability summary of one frozen procedure, not as a sampling interval over scientific units.
...
\begin{equation}
  ...
  \label{eq:sign-flip}
\end{equation}
Under the null hypothesis that each matched contrast is symmetrically distributed about zero, the contrast signs are exchangeable conditional on their magnitudes, which is what this test requires.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{27mm}>{\raggedright\arraybackslash}p{48mm}>{\raggedright\arraybackslash}p{59mm}@{}}
  \toprule
...
Distillation-headroom analysis &
  ...
  Seeds and blocks remain descriptive; the upper bound belongs to the relative contrast against the text-derived arm and is compared only with the continuation rule fixed before the results were seen \evd{E025} \\
  \midrule
  50-component linear target-projection assay &
...
For the primary retained-student movement audit, let \(H_s\) be the layer-6 representation matrix of the retained synthetic-target student and \(H_s^{\mathrm{KD}}\) that of its seed-matched ordinary-\ac{kd} student, both on the same \(n=1{,}999\) WikiText sentences held out from every training arm in technical seed \(s\), and let \(C=I-\mathbf{1}\mathbf{1}^{\top}/n\) center examples.\evd{E025}
...
The \result{e030_practical_threshold} unique-\(R^2\) rule was fixed before the transfer results were seen, to decide whether participant-level biological transfer justified a matched-control follow-up, and was then kept unchanged as the reference for the target-projection assay. \evd{E025}\evd{E030}
...
\end{equation}
Here \(\Delta\mathcal R_{\mathrm{semi}}^2=\mathcal R_F^2-\mathcal R_N^2\) is the population analogue of the cross-validated increment defined above.
...
Conditioning on \(S\) changes the question.
Because \(T\) is deterministic given \(S\), \(I(T;B\mid S)=0\).
...
Let \(p^\tau_{\mathrm{tch}}\) and \(q^\tau_{\mathrm{stu}}\) be the teacher and student output distributions at softmax temperature \(\tau\), let \(H_{\mathrm{tch}}\) and \(H_{\mathrm{stu}}\) be their internal states, let \(d_H\) be any declared discrepancy between those states, and let \(\Delta R_{\mathrm{transfer}}^2\) be the held-out biological-transfer contrast on recorded responses.
Then, without additional identifying assumptions,
\[
  D_{\mathrm{KL}}\!\left(p^\tau_{\mathrm{tch}}\middle\|q^\tau_{\mathrm{stu}}\right)=0
  \ \not\Rightarrow\
  d_H(H_{\mathrm{stu}},H_{\mathrm{tch}})=0,
  \qquad
