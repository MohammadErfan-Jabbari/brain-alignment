...
- **Never build this tree with tectonic.** The tracked `latexmkrc` pins `/usr/bin/biber`, which matches TeX Live 2023's biblatex 3.19 (control file 3.10). Tectonic ships biblatex 3.17 (control file 3.8), so alternating engines in this directory leaves a `.bcf` the other one's biber refuses, and silently swaps the PDF between 60 pages and 65. `manuscript_check.py` was switched to latexmk for that reason; do not reintroduce a tectonic branch.
- **Verify the engine, not only the page count:** `pdfinfo main-submission.pdf | grep Producer` must say `pdfTeX`. `xdvipdfmx` means a tectonic build slipped in. `717a9a7` shipped one; it was a complete 65-page PDF, not the bibliography-less build its fix commit `40505b7` claimed.
- **One `latexmk -pdf` run does not converge float placement after a reflow.** A prose change that frees a line can leave the page count two pages high until a second run settles the floats. Read the count only after two consecutive runs agree. On 2026-09-02 a single build reported 60 pages three times for a tree that converges to 58, which is how a two-page drop was briefly attributed to the wrong commit; isolating it needed `git worktree add --detach <commit>` and a two-pass build per commit.
- **If a build ever fails at biber, latexmk caches the failure and refuses to retry.** Delete `main-submission.bcf` and `main-submission.fdb_latexmk`, then rebuild. Under one engine the `.bbl` regenerates cleanly and is no longer load-bearing.
