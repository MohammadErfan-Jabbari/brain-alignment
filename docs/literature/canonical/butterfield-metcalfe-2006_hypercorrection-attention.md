...
This is **apparatus grounding for the agent's own `/teach` stance — NOT thesis science.** It touches none of A1/A2/A3, produces no brain-alignment number, uses none of our datasets, and must **not** appear in [`docs/01-research-landscape.md`](../../01-research-landscape.md)'s brain/distillation tables. Its single job is to let the `teach` skill (`.claude/skills/teach/SKILL.md`) *cite a mechanism* rather than assert it from parametric memory (the cite-or-flag spine).
