...
**Sibling note:** this is the *mechanism* follow-up to the original effect paper `butterfield-metcalfe-2001_hypercorrection-effect.md` (JEP:LMC 27(6):1491–1494) — read that note for the discovery; read this one for *why* it happens.
...
- `open-learner-models-and-errorful-learning.md` — the synthesis index that already lists hypercorrection under "high-confidence errors are corrected best"; this note is the dedicated mechanism source behind that line and behind the "diagnostic, not scorecard" framing.
- `bjork-2011_desirable-difficulties.md` — the prior-knowledge precondition (the favorable case is a knowledgeable adult who *can* be confidently wrong). `dunlosky-2013_effective-learning-techniques.md` — retrieval + spacing.
