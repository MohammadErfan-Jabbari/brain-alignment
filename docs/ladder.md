...
**Last updated:** 2026-06-29 (S45 — /work Q4 **E024: gaze-as-privileged-information → recorded NEGATIVE, gate-blocked.** Ran the LUPI sample-efficiency climb on ZuCo-NR through the binding positive-control gate (continuous opus review). **gaze ⊥ relation-label** — airtight null across every readout (binary AUC 0.51 perm-p 0.45; multiclass balanced-acc 0.20 perm-p 0.35; rich-35d+RF; within-subject); the *reliable* gaze features are a sentence-length confound. Substrate also structurally disqualified (only **7 paragraphs** → no stable paragraph-disjoint split; MDE ≥~10pp). Leak sharpening (Finding 3): **TSR** task-directed gaze predicts relation-type (balanced-acc 0.295, perm-p **0.003**) vs NR chance → gaze carries the relation only when actively hunted (a task-direction leak), not in natural reading. The 5-arm build was correctly NOT triggered; panel-converged (counter-argument SURVIVES-IF-NARROWED, stat-aggregation-auditor VERDICT-SAFE, Codex caught + fixed a reliability-deflation bug 0.51→0.71). **Q4/A3 stays ❌; the privileged-information/sample-efficiency axis is added to the robust null.** 8 commits (worktree `worktree-E024-sample-efficiency`, merged to main). — prior: S44 — /meta: **made the repo an Obsidian-native vault (still GitHub-clean).** Ratified the markdown conventions spec (`obsidian-conventions.md`); frontmatter + Related footers on 183 docs; references linkified repo-wide (330 + 268 path-links + 123 citation-links, clickable cites, citations→canonical notes); `.obsidian/` tracked + graph colour-groups; `.claude` exposed via symlinks; links-open-in-new-tab. 6 commits, all pushed. **NO experiment, NO science number, NO rung change — Q0–Q5 stand.** — prior: S43 — /meta: **D048 `/write` cutover EXECUTED — `sci-write-v2` is now the default `/write` engine.** P3-0 committed 12-scenario dress rehearsal (opus fixture-fairness audit caught + rebuilt 4 unfair fixtures) → P3-1 full **94-RUB acceptance suite graded in 7 batches → PASS + the 13 DET green, Erfan-signed** (suite `e49865ea3939f212`; 2 scenarios Erfan-adjudicated, SC-ARG-5 rewritten) → P3-2 cutover (routing repointed scientific-writing→sci-write-v2 across the live surface, old skill tombstoned read-only ~1wk, hook layer cut over: `stop_register_gate` retired so `stop_sw_converge` is the sole convergence gate, `prose_writecheck`→v2 `ai_tell_lint`), D048 amended COMPLETE. 14 commits. **NO experiment, NO science number, NO rung change — Q0–Q5 stand.** — prior: S42 — /meta: **pre-P3 checklist CLEARED — G1 RUB-grading harness BUILT (a–d); G2/G3
...
> **S45 DONE → E024 gaze-PI line STOPPED; next is science, Erfan's call (2026-06-29).** A `/work` climb on
> Q4's privileged-information line ran E024 to a recorded **NEGATIVE** through the binding positive-control gate:
> **gaze ⊥ relation-label** on ZuCo-NR (airtight; the reliable gaze features are a length confound), the
> substrate is structurally unsplittable (7 paragraphs), and gaze leaks the relation **only under task-directed
> reading** (TSR perm-p 0.003) — a task-direction artifact, not transferable cognition. **The 5-arm build was
> correctly NOT triggered. Q4/A3 stays ❌**, now with the sample-efficiency/gaze-PI axis added to the null.
> **Erfan-chosen path: stop the ZuCo gaze-relation line; the negative + the DPI bound + the control battery that
> caught it is the methodology contribution** (a `/write`/consolidation task, not `/work`; OneStop NOT pursued —
> transferability trap). 8 commits (worktree, merged to main). **Next options (Erfan picks):** **(a) `/write`**
> the deferred R07/R08 finding-reports through `sci-write-v2`; **(b) `/interpret`** the parked E006 voxelwise-CI
> item; **(c)** consolidate the E024 negative into the negative-results spine. Full record:
> [`experiments/E024_zuco-lupi-sample-efficiency.md`](experiments/E024_zuco-lupi-sample-efficiency.md); brain
> `projects/brain-alignment-e024-gaze-lupi-negative`; lessons L069/L070.
>
