...
> **S46 DONE → next is `/write` Section 4.3 / Q2 (2026-07-02).** The recent unwrapped manuscript sessions
> were closed retroactively. Figure 5 is finalized; Results §4.1 and §4.2 now follow the reader-facing
> pattern mined from prior feedback; Q0's confidence-interval wording is separated from the reliable-voxel
> positivity percentages; Q1 is framed as teacher-student headroom with a load-bearing quality/perplexity
> caveat. D054 changes close-session hygiene: after scoped wrap commits land, push committed work to
> `origin` by default. **NO experiment, NO science number, NO rung changed — Q0-Q5 stand.** **Next concrete
> step:** continue the Results pass on §4.3 (Q2/E004), preserving the honest inference-unit caveat:
> "undemonstrated, not proven zero." Keep the parked E006 CI-unit `/interpret` item separate unless the
> manuscript needs inferential wording the current records do not support.
>
