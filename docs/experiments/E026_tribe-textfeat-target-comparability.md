...
| MSE-student global relative parameter update | every seed has absolute log-ratio `<=0.10` where both relative updates exceed `1e-6`; otherwise absolute difference `<=1e-6`; blockwise values are descriptive |
| layer-6 CKA distance and centered-relative-Frobenius movement from KD | every seed has absolute CKA-distance difference `<=0.002`; absolute log Frobenius-movement ratio `<=0.10` when both exceed `10 ×` E025 duplicate noise, otherwise absolute difference `<=1e-6` |
...
- **Retrospective measured-axis check 1:** target geometry and learnability margins. **Check 2:** whether already-trained MSE students are parameter- and representation-movement-equivalent on heldout WikiText. Initial-gradient comparability remains separately unknown. Primary components are paired KD-only heldout target-R2 difference, sample stable rank and normalized top-spectrum mass, low-level/static-unique target predictability, and MSE-arm global parameter/layer-6 functional movement.
...
- Identifying assumption: if targets differ materially on baseline predictability, effective dimension/spectrum, or loss-induced movement, the between-target raw target-R2 gain cannot isolate target content. Passing comparability is necessary, not sufficient, for brain specificity.
...
- **Kill E027** if matching requires post-outcome adjustment, cannot achieve pretraining tolerances, fails the frozen one-shot movement gate, or copies item-wise TRIBE coordinates or the brain-derived readout. Do not launch a broad control sweep.
