...
The post-S81 claim-scope review selected the bounded real-brain robustness/rerun path before narrowing the paper. Its unique decision is retained here; the temporary review memo was removed after cutover.
...
### Step 51 - Bounded real-brain rerun launched (2026-07-07)

The predeclared TRIBE seed `0-2` artifact-saving rerun launched to `outputs/E016_tribe/phase3/phase3_rerun_tribe_gpt2_n95999_s0-2_lam0.1_save.json`, with saved students under `outputs/E016_tribe/phase3/model_artifacts/tribe_gpt2_n95999_s0-2_lam0.1_rerun/`. In parallel, textfeat seed `0-5` Tuckute scoring completed at `outputs/E016_tribe/phase3/phase3_combined_textfeat_gpt2_n95999_s0-5_lam0.1.tuckute_alignment.json` with all 18 expected artifact rows usable. The later result and audit are retained in Steps 68–70; routine process-monitoring details were removed during the source-of-truth cutover.
...
The `/interpret` closeout is consolidated below. A local independent recompute rebuilt the seed-aligned Tuckute contrasts from the raw alignment JSON rows and wrote `outputs/E016_tribe/phase3/phase3_combined_tribe_vs_textfeat_s0-5_tuckute_alignment.interpret_recompute.json`.
