...
**Created:** 2026-06-17 · **Status:** v4 RESOLVED (n=10 + learnability covariate) — **LIKELY POSITIVE, panel-pending (S22→S23). Surprisal-orthogonality NULL (raw≈residual). BUT human reading-time signal (raw≈residual) significantly beats ALL controls — permuted, random, log-freq, word-length, AND another LM's surprisal — and is ~46–48 AULC BELOW the non-cognitive learnability trend on every one of 10 seeds (CI excludes 0). The agent's auto-"clean null" was WRONG (it used a pooled regression that ignored the shared seed effect); the correct within-seed paired test shows the positive. NOT believed yet — Fork-A-adjacent, needs panel + replication + Erfan. NO rung flip.**
