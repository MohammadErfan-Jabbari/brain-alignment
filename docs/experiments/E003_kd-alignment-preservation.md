...
**Direction:** Q1 distillation-preservation gate; current interpretation is in the [canonical thesis manuscript](../manuscript/rewrite/main-rewrite.tex).
...
- **Matched budget:** batch size, max_length, and corpus matched across all trained arms. Compression ratio gpt2-medium→gpt2 ≈ **2.9×**. **Provenance correction (2026-06-17):** the optimizer-step count was *not* identical as-run — `kd_cold` ran 2 epochs (~12k steps), the warm arms (`kd_warm`, `lmft_warm`) 1 epoch (~6k steps) (`config.epochs` in the JSONs). So "matched budget" holds for corpus/batch/architecture but not step count; the cold arm got ~2× the steps. This *strengthens* the under-training caveat — the cold arm reached only ppl 477 *despite* the larger budget — and does not move any alignment number. (R07 still phrases the screen as "matched budget"; that is an analysis-lane wording flagged for Erfan, not edited here.) **Provenance correction (2026-09-02):** the learning rate was *not* matched either. `scripts/run_kd_alignment.py:352` selects `--cold-lr` (`3e-4`) for `kd_cold` and `--lr` (`5e-5`) for the warm arms, and `outputs/E003_cold.json` records `config.cold_lr=0.0003` as-run; the `dcfg.lr=5e-05` field is the shared template written to the JSON, not the arm's effective rate. The 2026-06-17 correction re-asserted "lr" while fixing the step count, so this is its second pass. **No recorded number moves.** A learning rate cannot change an already-measured alignment or perplexity value, and the asymmetry runs conservative: the cold arm received both a 6x larger rate and ~2x the steps and still reached ppl 477, which strengthens the under-training caveat rather than weakening it. Note for future readers that `kd_cold` is load-bearing twice, so this disclosure matters: the PARTIAL verdict is a `kd_cold` band read (rho'=0.37) and the log-ppl fit is cold-point-leveraged (`outputs/E003_dissociation.json` `leave_one_out_r.drop_kd_cold` = `-0.6418` against `-0.8835` with it). Neither depends on lr matching. The manuscript already states both rates and calls them separate optimization regimes.
