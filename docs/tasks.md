...
- [ ] **Phase 3 — cutover (X1–X4 now DONE; Erfan drives; IRREVERSIBLE; confirm with Erfan).** Run the FULL 135-scenario suite (`write-redesign-scenarios.md`); when green, retire the old draft flow (via git), wire the DET checks as always-on hooks, update `CLAUDE.md` + `docs/03-methodology.md`, record D048-complete. **Define first:** a RUB-grading harness / pass-threshold for the **~51 RUB scenarios** (incl. the 10 RUB `SC-XSTANCE-*` — see the P3 test-plan table in `write-redesign-scenarios.md`) + a sign-off mechanic (none exists beyond "Erfan signs"). NB: only the DET halves are machine-verified today — "135" is not "135 auto-passing." **Known residual — STILL OPEN:** the framing-sentence prose→lattice escape (an assertive sentence modeled as "framing" not a claim escapes the Trust floor; human gate is the only catch) — the design's prose→lattice claim-coverage check was not built; close it in P3 or accept it as a documented limit.
