...
- [x] **Literature route refreshed while rerun runs.** [`top-venue-literature-refresh-2026-07-07.md`](top-venue-literature-refresh-2026-07-07.md) records the current SOTA pressure: broad brain-tuning and privileged-information claims are occupied; our open cell is matched-control neural/cognitive PI distillation with real-brain transfer.
...
- [x] **Prepare seed-aligned Tuckute analyzer.** `scripts/e016_analyze_tuckute_alignment.py` regression-checks against the recorded seed `3-5` analysis and is ready for the post-rerun seed `0-5` diagnostic.
- [x] **Pre-audit waiting Tuckute postprocess path.** While the rerun remained active, the analyzer/audit scripts compiled, completed Tuckute row grids were checked, repeated alignment flags were verified as parser-supported, and the seed `3-5` analyzer/audit rehearsal reproduced the recorded warning route.
- [x] **Refresh PI/on-policy literature pressure with Firecrawl Research.** The S86/S87 continuation updated the literature refresh, evidence ledger, paper plan, and on-policy/context audit with July 2026 leakage/degradation pressure. This does not change the open cell, but it strengthens the real-brain-transfer gate and the controlled transfer-failure paper route if E016 stays nonpositive/mixed.
- [x] **Add combined Tuckute gate-status helper.** `scripts/e016_tuckute_gate_status.py` now gives a one-shot read-only answer for the real-brain transfer gate: missing files, selected runner-process presence, audit route when available, and whether `/interpret` can start.
- [x] **Refresh hidden-state distillation pressure.** Firecrawl Research checked NARF, OPRD, PHF, and the original BERT brain-tuning precursor. The paper route is now explicitly barred from claiming novelty for brain-guided LLM improvement or representation-level distillation; the live cell is brain-derived privileged targets under fixed-student KD controls and real-brain transfer.
- [x] **Rank post-positive representation controls.** [`top-venue-representation-control-feasibility-2026-07-07.md`](top-venue-representation-control-feasibility-2026-07-07.md) records that `contextfeat` is the first feasible non-brain control if the combined real-brain gate turns positive, while OPRD/PHF-style controls are new-runner work and should be `/precheck`-gated only after a robust positive.
- [x] **Arm-level rerun progress confirmed.** The stable rerun has now saved five model artifacts through `seed1_tribe_mse_lambda0p1` and entered `seed=1 arm=tribe_perm`; this is partial-arm progress only, not a result.
- [x] **Add venue-readiness matrix.** [`top-venue-readiness-matrix-2026-07-07.md`](top-venue-readiness-matrix-2026-07-07.md) maps the pending combined Tuckute gate to AAAI/ICML/ICLR/NeurIPS burden. Default route is AAAI for a clean nonpositive/mixed transfer-failure package; ICML/ICLR/NeurIPS require a robust positive plus stronger controls or a broader field-correcting negative.
- [x] **Refresh positive-branch privileged-OPD pressure.** Firecrawl Research expansion/readback added AR-OPD, EDGE-OPD, and When Context Returns to the on-policy/context audit. They sharpen the positive-branch reviewer objection around local support, leakage, evidence masking, and context removability, but do not change the immediate compute order: wait for the combined Tuckute gate; if positive, review first and build `contextfeat` before any new-runner OPD variant.
- [x] **Launch combined Tuckute postprocess watcher.** `outputs/E016_tribe/phase3/combined_tuckute_analysis_watcher_20260707.pid` records PID `3836562`; its log is `outputs/E016_tribe/phase3/combined_tuckute_analysis_watcher_20260707.log`. It waits for the rerun TRIBE Tuckute JSON and then runs `scripts/e016_analyze_tuckute_alignment.py` over TRIBE seed `0-5` and textfeat seed `0-5`.
...
- [x] **Saved-student Tuckute evaluator added.** [`scripts/e016_eval_saved_student_alignment.py`](../scripts/e016_eval_saved_student_alignment.py) reads an E016-style run JSON, loads saved `model_artifact_dir` students, and scores them on Tuckute with the E003 contiguous-CV nuisance-subtracted protocol. Outputs are diagnostics for `/interpret`, not verdicts.
- [x] **Real-brain probe docs wired into the paper ladder.** [`experiments/E016_tribe-synthetic-brain-targets.md`](experiments/E016_tribe-synthetic-brain-targets.md) Step 41, [`top-venue-paper-plan-2026-07-03.md`](top-venue-paper-plan-2026-07-03.md), [`top-venue-evidence-ledger-2026-07-03.md`](top-venue-evidence-ledger-2026-07-03.md), and [`scripts/AGENTS.md`](../scripts/AGENTS.md) now record the evaluator as branch-gated infrastructure only.
- [x] **Evaluator smoke passed.** Tiny CPU smoke loaded one saved `sshleifer/tiny-gpt2` student from the artifact-retention fixture, scored Tuckute with a tiny PCA setting, and wrote `outputs/E016_tribe/phase3/phase3_smoke_save_model_test.tuckute_alignment.json` with `artifact_rows_scored=1`.
...
- [ ] **If the positive branch needs post-hoc probes or real-brain evaluation, verify model artifacts first.** The active TRIBE full run started before `--save-model-dir` existed, so selected TRIBE arms may need a rerun with artifact saving before the Tuckute evaluator can be used at full scale.
...
- [x] **E016 readiness packet helper prepared.** [`scripts/e016_make_readiness_packet.py`](../scripts/e016_make_readiness_packet.py) extracts analyzer gate status, branch hint, paired effects, reviewer-burden flags, and next actions into a compact `/interpret` handoff packet without acting as a verdict engine.
...
- [x] **E016 watch-finalize helper added.** [`scripts/e016_watch_finalize_phase3.py`](../scripts/e016_watch_finalize_phase3.py) checks once by default, can poll with `--watch`, and only calls the guarded finalizer after the full run JSON exists. Smoke and active-run no-op paths are verified; it is not a verdict engine.
- [x] **Distillation-adjacency audit added.** [`top-venue-distillation-adjacency-audit-2026-07-03.md`](top-venue-distillation-adjacency-audit-2026-07-03.md) checks the closest LLM KD, feature-distillation, and privileged-information distillation neighbors. The paper plan now explicitly avoids claiming novelty for privileged-information KD itself and treats the matched-information `textfeat` control as central for any brain-specific positive branch.
- [x] **PI/feature-KD threat notes added.** Scout-grade canonical notes now exist for [`saadi-2026_task-tangent-feature-distillation-llms.md`](literature/canonical/saadi-2026_task-tangent-feature-distillation-llms.md), [`penaloza-2026_privileged-information-distillation-lms.md`](literature/canonical/penaloza-2026_privileged-information-distillation-lms.md), and [`wu-2026_pride-privileged-information-distillation-dialogue.md`](literature/canonical/wu-2026_pride-privileged-information-distillation-dialogue.md). [`01-research-landscape.md`](01-research-landscape.md), the adjacency audit, and the paper plan now record that PI-enhanced KD into smaller LMs is not novel; the open cell is specifically synthetic brain-alignment targets under matched fixed-budget KD controls.
- [x] **E016 post-finalizer branch router added.** [`scripts/e016_branch_decision.py`](../scripts/e016_branch_decision.py) reads the full run JSON, analyzer JSON, readiness JSON, and prepared text-feature launcher path, then prints the next safe command or stance. It returns `waiting_for_run_json`, `needs_finalizer`, `ready_for_interpret_controlled_null`, `textfeat_control_required`, `ready_for_target_control_comparison`, or `mixed_or_manual_interpretation_required`; it never launches training and is not a verdict engine.
- [x] **Privileged-signal adjacency audit added.** [`top-venue-privileged-signal-adjacency-audit-2026-07-03.md`](top-venue-privileged-signal-adjacency-audit-2026-07-03.md) checks context/self-distillation, rich-feedback distillation, and gaze/cognitive supervision neighbors. The paper plan now explicitly avoids claiming novelty for generic privileged/cognitive/context signals; the open cell remains controlled biological/synthetic-neural targets under fixed-budget text-KD.
- [x] **E016 post-positive reviewer-burden routing hardened.** [`scripts/e016_compare_target_controls.py`](../scripts/e016_compare_target_controls.py), [`scripts/e016_make_readiness_packet.py`](../scripts/e016_make_readiness_packet.py), and [`scripts/e016_branch_decision.py`](../scripts/e016_branch_decision.py) now carry the warning that TRIBE > textfeat is not brain-specific clearance without `/interpret`, stat/code review, and the privileged-signal adjacency follow-up burden.
- [x] **E016 target-control scope metadata added.** [`scripts/analyze_tribe_phase3.py`](../scripts/analyze_tribe_phase3.py) now preserves target-cache metadata, while [`scripts/e016_make_readiness_packet.py`](../scripts/e016_make_readiness_packet.py) and [`scripts/e016_compare_target_controls.py`](../scripts/e016_compare_target_controls.py) expose `target_cache_scope` / `target_scopes`. The paper plan now distinguishes the prepared sentence-local teacher-hidden-state `textfeat` control from unresolved long-context/on-policy distillation or real-brain follow-up.
- [ ] **Monitor active E016 Phase-3 training to analyzer JSON.** Current S71 close state: `phase=training_running_or_interrupted`, `health.status=runner_alive_log_quiet`, latest marker `seed=1`, `arm=tribe_mse`, `lambda=0.1`, `5/9` arm markers seen, `completed_arm_count=4`, `gpu.available=true`, `active_gpu_count=2`, `max_utilization_gpu_pct=59` in the instantaneous close-state sample, `run_json.exists=false`, `analysis_json.exists=false`. The completed-arm diagnostics are explicitly not Phase-3 results.
- [ ] **After analyzer JSON exists, use the branch router/readiness packet, then switch to `/interpret` only when ready.** Gate on completeness, all paired seeds, matched PPL, paired deltas, and sign-flip P-values before recording any verdict. Use `scripts/e016_branch_decision.py`, `scripts/e016_watch_finalize_phase3.py`, `scripts/e016_finalize_phase3.py`, or `scripts/e016_make_readiness_packet.py` as compact handoffs, not as verdict engines. Do not flip a rung without Erfan.
