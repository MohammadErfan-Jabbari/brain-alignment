...
- [ ] **TOOLING FIX — `start.json` clobber (L062).** A fresh `claude -p` verifier child can overwrite the parent's `.claude/state/wrap/start.json`. **Did NOT bite S38** (start.json was clean — correct session_id + start_sha), but the underlying fragility stands; the proper fix is a write-once-per-session SessionStart hook. `/wrap` works around it by checking `session_id` + falling back to first-commit diff. Lives in `~/.claude/` hooks, not this repo.
- [ ] **X1–X4 — cross-stance handoff (D050, BUILD-READY; S39). BUILD NEXT, BEFORE P3.** When a stage-5 reader finds a defect the prose can't fix (it lives in the evidence/argument substrate), `/write` routes it to the owning stance and blocks, instead of papering over. Full spec: `write-redesign-xstance.md`; build entry in `write-redesign-build-plan.md`. Three-net loop per chunk, order X1→X4: **X1** `evidence_status: suspect` at all 4 sites (`lattice_integrity.EVIDENCE_STATUS`, `claim_binding.STALE`+`check_register`, `LATTICE.md`, register) · **X2** `verdicts.py` `handoffs.json` store + `handoff open/resolve/status/check --lattice` + the `handoffs-open` clause in `status()` + `ship`/`accept-residual` refusal + no-op-when-missing selftest · **X3** SKILL stage-5.5 triage (reader self-tag, M3 backstop force-rule, handoff emit + human surface, two-gate rebind) · **X4** wire SC-XSTANCE-01 end-to-end + fold 16 `SC-XSTANCE-*` into the suite (119→135). Must land before P3 so cutover doesn't freeze the gap.
- [ ] **E006 voxelwise CI — parked `/interpret` item (S39 dry-run finding; NOT adjudicated).** The F13 panel showed the headline CI (`+0.021 [+0.0205, +0.0209]`) is a within-subject **voxel bootstrap** — pseudo-replicated (inferential n = 1 brain, not the voxel count; the L015 error, uncorrected on E006). The panel's fold-level recompute (still excludes zero, so the verdict likely survives) is a **lead to verify, NOT a fact**. A ✅-rung result (Q0/A2) reports it, so it matters. In `/interpret`: re-judge the bootstrap unit, restate the CI at fold level, set `evidence_status: suspect`/correct as warranted; then `/write` rebinds. **Do not run before X1–X4 is built** (the handoff is the path that surfaces it cleanly).
...
- [ ] **Phase 3 — cutover (AFTER X1–X4; Erfan drives; IRREVERSIBLE; confirm with Erfan).** Run the FULL 135-scenario suite (119 + 16 `SC-XSTANCE-*` after X4) (`write-redesign-scenarios.md`); when green, retire the old draft flow (via git), wire the DET checks as always-on hooks, update `CLAUDE.md` + `docs/03-methodology.md`, record D048-complete. **Define first:** a RUB-grading harness / pass-threshold for the ~41 RUB scenarios + a sign-off mechanic (none exists beyond "Erfan signs"). **Known residual — STILL OPEN:** the framing-sentence prose→lattice escape (an assertive sentence modeled as "framing" not a claim escapes the Trust floor; human gate is the only catch) — the design's prose→lattice claim-coverage check was not built; close it in P3 or accept it as a documented limit.
