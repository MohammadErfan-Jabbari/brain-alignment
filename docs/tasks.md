...
### ✍️ /write PIPELINE REBUILD — BUILD-READY (S35, 2026-06-22; D048; design in `docs/references/write-redesign-*`)
The redesign is complete and opus-reviewed; this is the build. Lift the 9 existing DET scripts (don't rewrite); the old `scientific-writing` draft flow stays live until Phase 3.
- [ ] **Phase 1 — walking skeleton.** Minimum vertical slice: `claim-lattice.json` (F15) + claim-binding (F3, lift `honesty_writecheck`+evd checks) + minimal skeleton (F6) + drafter-with-`\evd` (F8a) + two audits (F12 voice [lift `ai_tell_lint`+`prose-register-auditor`] + F9a/F9b fidelity) + gate (F16, AskUserQuestion). **Run on the abstract.** Acceptance: catches Claudio's class (SC-VOICE-01–04), passes the near-misses, every claim `\evd`-bound.
- [ ] **Phase 2 — full concern coverage.** F4/F5 (argument+scope judges, xhigh, 2 sites) · F11 (structure, 2 sites) · F18 (acknowledgment) · F13 (panel) · F19 (consistency) · F1 (reader-model) · F2 (message&frame) · F7 (figures) · F17 (exemplar set). CC-power: parallel stage-5 audits, structured-output verdicts (`ready_to_ship`), Stop-hook convergence.
- [ ] **Phase 3 — cutover.** Run the full 119-scenario suite (`write-redesign-scenarios.md`); when green, retire the old draft flow, update `CLAUDE.md` + `docs/03-methodology.md`, record the cutover.
