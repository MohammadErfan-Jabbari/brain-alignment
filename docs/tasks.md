...
- [ ] **PRE-P3 READINESS CHECKLIST (clear these before the cutover can RUN).** X1–X4 verified (S41); the design front is unblocked. What still stands between us and P3, in order:
  - **G1 — RUB-grading harness (THE blocker; `/meta`).** The ~51 RUB scenarios (41 prior + 10 RUB `SC-XSTANCE-*`) have no scorer — only the DET halves are machine-verified, so "135" is NOT "135 auto-passing." Sub-steps: **(a)** define the grading protocol — a *fresh* judge runs each RUB scenario's INPUT through its functionality and checks the stated EXPECT/pass-criterion → PASS/FAIL (reuse the per-functionality judges; this is a "run-the-scenario" harness, not a new judge); **(b)** define the pass-threshold (proposal: every DET green **and** every RUB PASS, with the anchors SC-VOICE-01..04 and SC-XSTANCE-01 hard-required and never waivable); **(c)** define the sign-off mechanic (Erfan reviews the RUB tally + signs — today "Erfan signs" is undefined); **(d)** optionally build a runner script + a results-record format. Start from the **P3 test-plan table in `write-redesign-scenarios.md`** (it already maps each scenario to its DET label or its RUB criterion). *Decide at P3 start whether to scope this design before or as the first P3 step.*
  - **G2 — framing-sentence escape: a DECISION, not a drift.** Either **close** it (build the prose→lattice claim-coverage check — a load-bearing sentence not entered as a claim gets flagged; `/meta` build) **or** record a decision to **accept** it as a documented cutover limit (human gate is the catch). Don't leave it implicit through the cutover.
  - **G3 (optional confidence, not a gate) — a converged real-section demo.** Run the full pipeline *including the revise loop to convergence* on one real section (finish the S39 dry-run draft of R06/§4.1, separate from the E006 handoff), to confirm the loop closes and ships good prose. Cheap confidence before the irreversible step.
...
- [ ] **Phase 3 — cutover (after G1 + G2 decided; Erfan drives; IRREVERSIBLE; confirm before each step).** Run the FULL 135-scenario suite (DET machine-green **+** RUB harness-pass per G1); when green: retire the old `scientific-writing` flow (via git), wire the DET checks as always-on hooks, update `CLAUDE.md` + `docs/03-methodology.md` to point at `sci-write-v2`, record **D048-complete**.
