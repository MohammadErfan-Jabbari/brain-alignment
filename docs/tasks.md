...
- [x] **E016 status helper hardened for the live run.** [`scripts/e016_phase3_status.py`](../scripts/e016_phase3_status.py) now reports ETA fields, train/heldout stage-aware progress, training-arm markers, and the active `run_tribe_phase3.py` runner processes.
...
- [ ] **Monitor active E016 Phase-3 training to analyzer JSON.** Current S53 close state: `phase=training_running_or_interrupted`, latest marker `seed=0`, `arm=kd_only`, `lambda=0.0`, `1/9` arm markers seen, `run_json.exists=false`, `analysis_json.exists=false`.
