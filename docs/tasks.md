...
- [ ] **P2-B — NEXT.** F11 structure judge (2 sites) + F1 reader-model + F2 message&frame. Verify SC-STR-01..12, SC-RM-1/2.
- [ ] **P2-C.** F18 acknowledgment + F13 premortem-panel wiring + F7 figures (sets `meta.figures_planned`) + F17 exemplar pin + F8b voice-realize + F19 consistency (lift `check_number_consistency`).
- [ ] **P2-D — CC-power upgrades.** Parallel stage-5 fan-out + structured `ready_to_ship` verdicts + D047 Stop-hook convergence + AskUserQuestion gate. (The judges already emit `*-VERDICT: {ready_to_ship}` lines.)
- [ ] **Phase 3 — cutover (IRREVERSIBLE; confirm with Erfan).** Run the FULL 119-scenario suite (`write-redesign-scenarios.md`) + opus-designed additions; when green, retire the old draft flow (via git), wire the DET checks as always-on hooks, update `CLAUDE.md` + `docs/03-methodology.md`, record D048-complete. **Known residual to close in P2-B/C:** an assertive sentence the drafter models as "framing" (not a claim) escapes the Trust floor today — only the human gate catches it (marked in SKILL.md).
