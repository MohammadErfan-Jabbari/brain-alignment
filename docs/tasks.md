...
- [ ] **Build the `taste-reader` subagent** (Erfan, S33) — a dedicated opus `.claude/agents/taste-reader.md` that judges paragraph READABILITY/TASTE, distinct from `prose-register-auditor` (rule-level tells). Wire into the `scientific-writing` review pass. Run inline per paragraph until built.
