#!/usr/bin/env node
// Section review swarm generator (v3) — folds Section-3 learnings into prompts and process.
// Lineage: v1 ran Section 2 (9 children, /tmp/s2-swarm.js), v2 ran Section 3 (17 children,
// /tmp/s3-swarm.js + /tmp/s3-round2.js + /tmp/s3-round2-judge.js). v3 = v2 + learnings below.
//
...
- Table cell text and captions are in scope; tikz code is out of scope.`,
    ...
    scopes: [
      { key: 'pre41', anchor: '\\subsection{Controlled brain predictivity under regional and naturalistic assays}', scope: `the section preamble (from \\section{Results} up to subsection 4.1) AND subsection 'Controlled brain predictivity under regional and naturalistic assays' (including any table cell text and captions; not tikz code). Number prose paragraphs P1.. in order across preamble then subsection.` },
      { key: 's42', anchor: '\\subsection{Observed distillation headroom and its association with language quality}', scope: `subsection 'Observed distillation headroom and its association with language quality' (including any table cell text and captions; not tikz code). Number prose paragraphs P1.. in order.` },
      { key: 's43', anchor: '\\subsection{Recorded-response intervention at valid inference units}', scope: `subsection 'Recorded-response intervention at valid inference units' (including any table cell text and captions; not tikz code). Number prose paragraphs P1.. in order.` },
      { key: 's44', anchor: '\\subsection{Measurability of the 50-component linear target projection}', scope: `subsection 'Measurability of the 50-component linear target projection' (including any table cell text and captions; not tikz code). Number prose paragraphs P1.. in order.` },
      { key: 's45', anchor: '\\subsection{Synthetic-response manipulation checks and attribution status}', scope: `subsection 'Synthetic-response manipulation checks and attribution status' (including any table cell text and captions; not tikz code). Number prose paragraphs P1.. in order.` },
      { key: 's46', anchor: '\\subsection{Saved-student biological-transfer assays}', scope: `subsection 'Saved-student biological-transfer assays' (including any table cell text and captions; not tikz code). Number prose paragraphs P1.. in order.` },
      { key: 's47', anchor: '\\subsection{Practical-utility evaluation}', scope: `subsection 'Practical-utility evaluation' (including any table cell text and captions; not tikz code). Number prose paragraphs P1.. in order.` },
    ],
  },
};
...
function extractSection(cfg) {
  ...
  for (const s of cfg.scopes) {
    ...
    if (hits !== 1) throw new Error(`anchor for scope ${s.key} found ${hits} times (expected 1): ${s.anchor}`);
  }
  return full;
}

function buildContext(cfg) {
  return [SHARED.thesisCtx, SHARED.profLessons, SHARED.claudio, SHARED.settled,
          SHARED.roleTerms, SHARED.deferred, cfg.rules, SHARED.hard].join('\n\n');
}

function stamp(label, text, kind) {
  return `// ${label} ${kind} — generated ${new Date().toISOString()} by .claude/scripts/section_swarm.js (v3)\n` +
...
const SUBS = ${JSON.stringify(cfg.scopes.map(({ key, scope }) => ({ key, scope })), null, 0)};
const CRITERIA = ${JSON.stringify(cfg.criteria)};

function analystPrompt(sub) {
  return CONTEXT + "\\n\\nYOUR TASK. You are reviewing ONE part of ${L} of the thesis, paragraph by paragraph, with the whole section below for context. Review ONLY: " + sub.scope + "\\n\\n" +
  "Give every finding a unique ID: " + sub.key + "-P<paragraph>.<k> (k = 1, 2, ... within that paragraph). Counter-reviewers and judges will refer to these IDs; a malformed ID cannot be applied.\\n\\n" +
  ...
  "ID: " + sub.key + "-P<n>.<k> TYPE=<term-first-use|storytelling|passive|readability|flow|number-fidelity|verdict-fidelity|consistency|precision|design-fact> SEVERITY=<high|med|low>\\n" +
  "QUOTE: <verbatim fragment from the text>\\n" +
  "PROBLEM: <one sentence>\\n" +
  "PROPOSAL: <exact replacement LaTeX text, or NONE-keep>\\n" +
  "RATIONALE: <one sentence>\\n" +
  "If a paragraph has no defect write: " + sub.key + "-P<n> CLEAN.\\n" +
...
function counterPrompt(sub, analystOut) {
  return CONTEXT + "\\n\\nYOUR TASK. You are the adversarial counter-reviewer for: " + sub.scope + "\\n\\n" +
  "A first reviewer proposed the changes below, each with an ID. Argue, per proposal, whether it IMPROVES or WORSENS the manuscript, and catch defects the first reviewer missed. ${cfg.guardian}\\n\\n" +
  ...
  "Then add: MISSED: up to 3 defects the first reviewer missed in this scope, same block format the first reviewer used, with fresh IDs " + sub.key + "-M<k> (or 'none').\\n" +
  "DO NOT EDIT ANY FILE; your output is your only product.\\n\\n" +
  "FULL SECTION TEXT (context):\\n\\n" + SECTION + "\\n\\nFIRST REVIEWER'S PROPOSALS FOR YOUR SCOPE:\\n\\n" + analystOut;
}

// ---- Phase 1: analysts in parallel (review-only) ----
const analystRuns = SUBS.map(s => ({ key: "a-" + s.key, agent: "reviewer", task: analystPrompt(s) }));
const analystResults = await runs.all(analystRuns);
const analystOut = {};
SUBS.forEach((s, i) => {
  const r = analystResults[i];
  analystOut[s.key] = (typeof r === "string" ? r : (r && (r.output || r.result)) || JSON.stringify(r)).slice(0, 14000);
});
emit("PHASE1 DONE: " + SUBS.map(s => s.key + "=" + analystOut[s.key].length + "ch").join(", "));

// ---- Phase 2: adversarial counter-reviewers in parallel ----
const counterRuns = SUBS.map(s => ({ key: "c-" + s.key, agent: "reviewer", task: counterPrompt(s, analystOut[s.key]) }));
const counterResults = await runs.all(counterRuns);
const counterOut = {};
SUBS.forEach((s, i) => {
  const r = counterResults[i];
  counterOut[s.key] = (typeof r === "string" ? r : (r && (r.output || r.result)) || JSON.stringify(r)).slice(0, 14000);
});
emit("PHASE2 DONE: " + SUBS.map(s => s.key + "=" + counterOut[s.key].length + "ch").join(", "));

// ---- Phase 3: two independent judges in parallel (decorrelated providers) ----
const jBundle = SUBS.map(s => "\\n\\n########## SCOPE " + s.key + " (" + s.scope + ") ##########\\n--- FIRST REVIEWER PROPOSALS ---\\n" + analystOut[s.key] + "\\n--- COUNTER-REVIEWER VERDICTS ---\\n" + counterOut[s.key]).join("");
const judgeBody = CONTEXT +
  "\\n\\nYOUR TASK. You are an independent judge for a revision of ${L} of the thesis. Below, for each scope, are a first reviewer's proposals and an adversarial counter-reviewer's verdicts; every item carries a unique ID.\\n" +
...
const judgeResults = await runs.all([
  { key: "judge-deepseek", agent: "reviewer", model: "deepseek/deepseek-v4-pro:medium", task: judgeBody },
  { key: "judge-kimi", agent: "reviewer", model: "kimi-coding/k3", task: judgeBody }
]);
const judge1 = (typeof judgeResults[0] === "string" ? judgeResults[0] : (judgeResults[0] && (judgeResults[0].output || judgeResults[0].result)) || JSON.stringify(judgeResults[0])).slice(0, 25000);
const judge2 = (typeof judgeResults[1] === "string" ? judgeResults[1] : (judgeResults[1] && (judgeResults[1].output || judgeResults[1].result)) || JSON.stringify(judgeResults[1])).slice(0, 25000);
emit("PHASE3 DONE: judge1 " + judge1.length + "ch, judge2 " + judge2.length + "ch");

return { analyst: analystOut, counter: counterOut, judge1: judge1, judge2: judge2 };
`;
}
...
function main() {
  ...
  if (!SECTIONS[n]) {
    console.error('No config for section ' + secArg + '. Configured: ' + Object.keys(SECTIONS).join(', '));
    console.error('Add an entry to SECTIONS in .claude/scripts/section_swarm.js when its review turn comes.');
    process.exit(1);
  }
  ...
  if (mode === '--verify') {
    ...
    if (!m) { console.error('No sha256 stamp found in ' + arg3); process.exit(1); }
    console.log('live: ' + live);
    console.log('embed: ' + m[1]);
    console.log(live === m[1] ? 'MATCH — embedded text is byte-identical to the live file' : 'MISMATCH — regenerate before launching');
  ...
  if (mode === '--round2') {
    ...
    if (!arg3 || !fs.existsSync(arg3)) { console.error('findings file missing: ' + arg3); process.exit(1); }
