...
3. **Doc consistency / source-of-truth audit.** Do `ladder.md`, `upspeed.md`, `tasks.md`, and the relevant report (R03/R04/…) agree? Any number stated in a doc that is *not* backed by an `experiments/` entry (the D011 cite-or-flag rule)? Any claim this session contradicted that is now stale? Fix it, or flag it loudly in the timeline if the fix needs Erfan.
   - **Manuscript ship-gate (D046).** If `docs/manuscript/` was edited this session, confirm the `scientific-writing` full loop actually ran on it — the non-skippable review pass, which spawns `prose-register-auditor`, included. Run `uv run python .claude/skills/scientific-writing/scripts/ai_tell_lint.py <edited files>` and report any hard findings or soft register/passive warnings. A manuscript edit that bypassed the loop (the v0.1 failure mode, L054) is **not shippable** — flag it loudly; the prose owes the auditor before it reaches a human. A clean deterministic run is not a clearance.
