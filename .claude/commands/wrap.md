...
1. **Record which stances ran** — the dominant one and any others the session crossed into. It frames everything.
...
4. **REPLACE `docs/upspeed.md`** — short, framed by the dominant stance (`/work`: what ran / next to run; reporting stances: what's understood / written / figured).
5. **Update `docs/tasks.md`** — move finished to Done with a date; add new tasks.
6. **`docs/ladder.md`** — flip changed rungs, record verdicts (specific numbers + named test; ✅ only on an `experiments/`-recorded result, partials stay 🟡 with the caveat), update supporting tracks, **rewrite the "Next session" block** (the next stance + the single concrete next step). **Get Erfan's confirmation before flipping any rung.**
...
3. **Doc consistency / source-of-truth audit.** Do `ladder.md`, `upspeed.md`, `tasks.md`, and the relevant report (R03/R04/…) agree? Any number stated in a doc that is *not* backed by an `experiments/` entry (the D011 cite-or-flag rule)? Any claim this session contradicted that is now stale? Fix it, or flag it loudly in the timeline if the fix needs Erfan.
   - **Manuscript register state (informational — `/wrap` does NOT own this, D047).** Write-quality convergence is enforced at the write actuator by the `Stop` register gate, not here; `/wrap` only *reports*. If `docs/manuscript/` or `docs/reports/` changed this session, read `.claude/state/register/verdict.json` and state in the close summary whether each edited deliverable carries a clean verdict for its current bytes (or an accepted-residual sign-off). If one does not, note it as an open loop — but do not try to gate or fix it here; that belongs to a `/write` session and its `Stop` gate. (`/wrap` is a generic session finalizer; it must not become the write-stance quality owner.)
...
5. **Folder `AGENTS.md` freshness.** Did this session change a folder's structure, commands, workflow, conventions, or recurring traps? If yes, update the nearest `AGENTS.md` now. Root `README.md` is the only README; never create a non-root README as a folder guide.
6. **Ladder integrity.** Every rung status matches an `experiments/` record; partials carry their caveat; the "Next session" block is concrete and stance-tagged; kill criteria still predeclared for open rungs. (This is the one file the whole continuity process trusts.)
7. **Brain mirror.** Was any durable decision / finding / idea mirrored to gbrain (per the global brain-first protocol)? If a `put_page` is warranted and not done, do it (provenance `author: agent`; never secrets), or say why it's pure ephemeral plumbing.
8. **Git hygiene.** Is every meaningful step committed atomically with a conventional message (D007), scoped staging only (never `git add -A`/`.`)? Is the working tree clean, with nothing important left uncommitted? Run `git status --short` and report. After the wrap commit(s) land, push committed work to `origin` by default unless Erfan explicitly says not to or the push is blocked; report the push result.
9. **Open loops.** Any background job still running, half-finished edit, or unverified result? Surface it explicitly in `upspeed.md` Blockers/Next so it isn't silently lost.
...
2. **Spawn the auditors in parallel** — one `wrap-auditor` per scope, in a single message so they run concurrently. Each gets its `SCOPE`, the `git diff <start_sha>..HEAD` + `git status`, and (for `records-completeness` and the friction note) the transcript slice. Model routing: **sonnet for all five by default** — they are scoped doc-nav checks and the opus orchestrator re-verifies every finding, so the reasoning backstop is centralized, not duplicated. Escalate **`ladder-integrity` to opus only when an experiment ran this session** and a rung flip is genuinely in question (an independent opus skeptic on a real verdict earns its cost; on a non-`/work` session it does not). Use haiku for a purely mechanical scope if you split one out. The five scopes (`ladder-integrity`, `number-provenance`, `continuity-docs`, `records-completeness`, `git-and-artifacts`) together cover Part B's checks.
...
The swarm finds and proposes; you confirm, write, commit, and push the committed wrap work to `origin` by default. The invariants are unchanged: never invent progress, never flip a rung without Erfan, never `git add -A`.

## Return

A short close summary: the **stances** run; the **tier** chosen (and the fallback note if `start.json` was missing); on a heavy session, the **swarm findings** (applied vs dropped-as-unfounded); the timeline log path; the **proposed ladder change** (and confirm it with Erfan if a rung flips); the new `upspeed.md` "next" step; any new learning/decision; and the **continuity-audit findings** — friction + proposed fixes, tooling issues, doc inconsistencies fixed/flagged, any new-artifact proposal, git status, and push result. Keep numbers specific. End by stating clearly what (if anything) needs Erfan's confirmation before it's final.
