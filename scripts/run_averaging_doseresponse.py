...
def main():
    ...
    results = {"model": args.model, "nc": nc, "uid_order": UID_ORDER, "config": vars(args), "raw": []}
    t0 = time.time()
    for k in args.k:
        uids_k = UID_ORDER[:k]
        Yk = np.mean([per_uid[u] for u in uids_k], axis=0)   # k-averaged target
        Yk_tr, Yk_ev = Yk[tr], Yk[ev]
        nc_k = nc / (nc + (1 - nc) / k)                       # predicted achievable ceiling
        for seed in args.seeds:
            for arm in ("mse", "mse_perm"):
                P.set_seed(seed)
                m, _ = RBL.load_model(args.model, device)
                Yt = Yk_tr if arm == "mse" else BL.block_permute(Yk_tr, n_blocks=10, seed=1000 + k * 17 + seed)
                ...
                results["raw"].append({"k": k, "seed": seed, "arm": arm, "unique_r2": uR2, "nc_k_pred": nc_k})
                print(f"  [k={k} {arm:9s} s{seed}] uR2(vs k-avg target)={uR2:+.4f}  (pred ceiling {nc_k:.3f})", flush=True)
                del m
    ...
    print(f"{'k':>3s} {'gap (mse-perm)':>15s} {'sd':>8s} {'n+':>5s} {'pred ceiling':>13s}")
    from collections import defaultdict
    bykseed = defaultdict(dict)
    for r in results["raw"]:
        bykseed[(r["k"], r["seed"])][r["arm"]] = r["unique_r2"]
    for k in args.k:
        gaps = [bykseed[(k, s)]["mse"] - bykseed[(k, s)]["mse_perm"] for s in args.seeds]
        nc_k = nc / (nc + (1 - nc) / k)
        sd = float(np.std(gaps))
        print(f"{k:>3d} {np.mean(gaps):>+15.5f} {sd:>8.5f} {sum(1 for g in gaps if g>0):>3d}/{len(gaps)} {nc_k:>13.3f}")
    print("PREDICTION: if averaging manufactures specificity, gap(k) rises from ~0 at k=1 toward +0.008 at k=5, monotone in k.")
