...
| `figures/` | Figure generation scripts. `figures/make_brain_figure.py` builds the defense-deck LH language-network figure (EvLab SN220 parcels on fsaverage pial) into `outputs/presentations/brain-lh-language/`, downloading the parcel volume to `data/evlab-parcels/` on first run. |
