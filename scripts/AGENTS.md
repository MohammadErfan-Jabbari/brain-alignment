...
| `manuscript_check.py` | Deterministic manuscript gate: evidence-marker resolution, keyed-number resolution across the `\input` graph, bare result-like numbers, `\gap` survival under `--share-ready`, and the LaTeX build. Moved here from `.claude/` by D068. |
