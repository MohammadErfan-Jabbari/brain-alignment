...
| `figures/` | Figure generation scripts. `figures/make_brain_figure.py` builds the defense-deck LH language-network figure (EvLab SN220 parcels on fsaverage pial) into `outputs/presentations/brain-lh-language/`, downloading the parcel volume to `data/evlab-parcels/` on first run. `figures/make_tribe_response_figure.py` renders one TRIBE v2 synthetic response (E016 phase-3 route, `predict` stage in the TRIBE venv, `render` stage in the thesis env) on fsaverage5 into `outputs/presentations/tribe-response/`. |
